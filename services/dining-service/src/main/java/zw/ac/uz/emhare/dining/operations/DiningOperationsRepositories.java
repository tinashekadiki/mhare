...
/** @author Tinashe K */
interface OperationalDiningHallRepository extends JpaRepository<DiningHall,UUID>{}
interface OperationalDiningPlanRepository extends JpaRepository<DiningPlan,UUID>{}
interface OperationalMealOptionRepository extends JpaRepository<MealOption,UUID>{}
interface OperationalDiningPlanMealRepository extends JpaRepository<DiningPlanMeal,UUID>{Optional<DiningPlanMeal> findByDiningPlanIdAndMealOptionId(UUID planId,UUID mealOptionId);}
interface StudentDiningAssignmentRepository extends JpaRepository<StudentDiningAssignment,UUID>{
...
interface StudentDietaryRequirementRepository extends JpaRepository<StudentDietaryRequirement,UUID>{List<StudentDietaryRequirement> findAllByDeletedAtIsNullOrderByStudentNumberAscRequirementCodeAsc();}
interface MealServiceSessionRepository extends JpaRepository<MealServiceSession,UUID>{
...
interface MealAttendanceEventRepository extends JpaRepository<MealAttendanceEvent,UUID>{
    ...
    @Query("select count(event) from MealAttendanceEvent event where event.session.id=:sessionId and event.outcome=zw.ac.uz.emhare.dining.operations.MealAttendanceEvent.Outcome.ADMITTED and not exists (select reversal.id from MealAttendanceReversal reversal where reversal.event=event)")
...
interface MealAttendanceReversalRepository extends JpaRepository<MealAttendanceReversal,UUID>{
    List<MealAttendanceReversal> findAllByDeletedAtIsNullOrderByReversedAtDesc();
    boolean existsByEventId(UUID eventId);
}
interface DiningWorkflowEventRepository extends JpaRepository<DiningWorkflowEvent,UUID>{List<DiningWorkflowEvent> findAllByDeletedAtIsNullOrderByOccurredAtDesc();}
