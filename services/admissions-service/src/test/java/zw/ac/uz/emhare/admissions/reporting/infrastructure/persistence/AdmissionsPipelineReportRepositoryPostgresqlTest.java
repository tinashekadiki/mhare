...
class AdmissionsPipelineReportRepositoryPostgresqlTest {
    ...
    void projections_shouldReadCurrentApplicationsAndApplyProgrammeAndDemographicFilters() {
        ...
        UUID applicationTypeId = jdbcTemplate.queryForObject(
        ...
        UUID localApplicationId = insertApplication(localApplicantId, applicationTypeId, intakeId, "APP-LOCAL");
        UUID internationalApplicationId = insertApplication(
        ...
        insertChoice(localApplicationId, computerScienceProgrammeId, 2, "HCS", "Bachelor of Computer Science");
        insertChoice(internationalApplicationId, computerScienceProgrammeId, 1, "HCS", "Bachelor of Computer Science");
        jdbcTemplate.update("UPDATE applications SET status = 'UNDER_REVIEW'");

        List<AdmissionsPipelineReportRow> allRows = repository.findReportRows(AdmissionsPipelineReportQuery.empty());
        List<AdmissionsPipelineReportRow> localComputerScience = repository.findReportRows(
        ...
        assertEquals(List.of("INTERNATIONAL", "LOCAL"), repository.findFilterOptions().categories().stream()
                .map(option -> option.code()).toList());
    }

    @Test
    void detailedExport_shouldReturnOneApplicationRowAndKeepAllRankedChoicesWhenFilteringByProgramme() {
        UUID intakeId = UUID.randomUUID();
        UUID applicationTypeId = jdbcTemplate.queryForObject(
                "SELECT id FROM application_types WHERE code = 'UNDERGRAD'", UUID.class);
        ...
        UUID draftApplicationId = insertApplication(
                draftApplicantId, applicationTypeId, intakeId, "APP-DRAFT");
        UUID accountingProgrammeId = UUID.randomUUID();
        UUID computerScienceProgrammeId = UUID.randomUUID();
        insertChoice(applicationId, accountingProgrammeId, 1, "HACC", "Bachelor of Accountancy");
        insertChoice(applicationId, computerScienceProgrammeId, 2, "HCS", "Bachelor of Computer Science");
        jdbcTemplate.update("""
                UPDATE applications
        ...
        List<AdmissionsDetailedExportRow> rows = repository.findDetailedExportRows(
                AdmissionsPipelineReportQuery.of(
                        intakeId, computerScienceProgrammeId, applicationTypeId, "local", "female"));
        ...
        assertEquals("1. HACC - Bachelor of Accountancy | 2. HCS - Bachelor of Computer Science",
                row.programmeChoices());

        AdmissionsDetailedExportRow draftRow = repository.findDetailedExportRows(AdmissionsPipelineReportQuery.empty())
                .stream().filter(item -> draftApplicationId.equals(item.applicationId())).findFirst().orElseThrow();
    ...
    void operationalProjection_shouldExposeCanonicalApplicantChoiceAndWorkflowEvidence() {
        ...
        jdbcTemplate.update("UPDATE applicants SET disability_status_code = 'VISUAL_IMPAIRMENT', sponsor_type_code = 'STAFF_DEPENDANT' WHERE id = ?", applicantId);
        UUID applicationId = insertApplication(applicantId, applicationTypeId, intakeId, "APP-OPERATIONAL");
        UUID programmeId = UUID.randomUUID();
        insertChoice(applicationId, programmeId, 1, "HCS", "Bachelor of Computer Science");

        List<AdmissionsOperationalReportRow> rows = operationalRepository.findRows(
                AdmissionsPipelineReportQuery.of(intakeId, programmeId, applicationTypeId, "local", "female"));
    ...
    void intakeMovementProjection_shouldReconstructBeforeAndAfterIntakesAndApplyEveryFilter() {
        ...
        UUID applicationId = insertApplication(applicantId, applicationTypeId, previousIntakeId, "APP-MOVED");
        UUID programmeId = UUID.randomUUID();
        insertChoice(applicationId, programmeId, 1, "HCS", "Bachelor of Computer Science");
        jdbcTemplate.update("""
                UPDATE applications
                 ...
                 WHERE id = ?
                """, newIntakeId, applicationId);
        UUID actor = UUID.randomUUID();
        Integer firstRevision = jdbcTemplate.queryForObject("""
                INSERT INTO revinfo (revtstmp, actor_user_id, reason)
                VALUES (1786694400000, ?, 'Initial intake') RETURNING rev
                """, Integer.class, actor);
        Integer secondRevision = jdbcTemplate.queryForObject("""
                INSERT INTO revinfo (revtstmp, actor_user_id, reason)
        ...
        jdbcTemplate.update("""
                ...
                """, applicationId, firstRevision, previousIntakeId,
                applicationId, secondRevision, newIntakeId);

        List<AdmissionsIntakeMovementRow> movements = operationalRepository.findIntakeMovements(
                AdmissionsPipelineReportQuery.of(
    ...
    private UUID insertApplicant(String categoryCode, String genderCode, String namePrefix) {
        ...
        jdbcTemplate.update("""
                ...
                """, applicantId, UUID.randomUUID(), "A" + applicantId.toString().substring(0, 6), categoryCode,
                namePrefix, genderCode, applicantId + "@example.test");
        return applicantId;
    }

    private UUID insertApplication(UUID applicantId, UUID applicationTypeId, UUID intakeId, String numberPrefix) {
        UUID applicationId = UUID.randomUUID();
        jdbcTemplate.update("""
                INSERT INTO applications (
                    id, applicant_id, application_type_id, application_number, payment_required,
                    status, created_at, updated_at, version, intake_id, intake_code, intake_name,
                    intake_starts_on, intake_ends_on, maximum_programme_choices
                ) VALUES (?, ?, ?, ?, true, 'DRAFT', now(), now(), 0, ?, 'AUG-2026', 'August 2026',
                          DATE '2026-08-01', DATE '2026-08-31', 3)
                """, applicationId, applicantId, applicationTypeId,
                numberPrefix + '-' + applicationId.toString().substring(0, 6), intakeId);
        return applicationId;
    }

    private void insertChoice(UUID applicationId, UUID programmeId, int rank, String code, String name) {
        jdbcTemplate.update("""
                ...
                """, UUID.randomUUID(), applicationId, programmeId, rank, code, name);
    }
}
