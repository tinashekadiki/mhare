...
class AdmissionsWorkItemServiceTest {
  ...
  @Test
  void
      availableActions_shouldAllowDecisionAndReturnForCurrentChoice_whenEarlierChoiceWasRejected() {
    ...
    when(decisionRepository.existsByProgrammeChoiceIdAndDeletedAtIsNull(currentChoiceId))
        .thenReturn(false, true);

    List<String> actions =
        availableActions(application, review, recommendation, admissionsOfficerProfile());
  ...
  void availableActions_shouldOfferOneRevisedRecommendationAction_whenAdmissionsReturnsTheReview() {
    ...
    when(returnedRecommendation.getReviewStatus())
        .thenReturn(AcademicRecommendationReviewStatus.RETURNED);

    List<String> actions =
        availableActions(
            application, review, returnedRecommendation, academicReviewerProfile(academicUnitId));
  ...
  void caseBlockers_shouldSurfaceFailedDuplicateEvidenceForSubmittedApplication() {
    ...
    when(choiceRepository.findAllByApplicationIdOrderByChoiceRankAsc(application.getId()))
        .thenReturn(List.of());
    when(duplicateCheckService.check(application))
        .thenReturn(
            new ApplicationDuplicateCheckService.DuplicateCheckResult(
                false, "Duplicate intake application found."));
  ...
  @Test
  void
      availableActions_shouldNotReopenPendingAcademicRecommendationOrDecideWithoutRecommendation() {
    UUID academicUnitId = UUID.randomUUID();
    Application application = mock(Application.class);
    AcademicReview review = mock(AcademicReview.class);
    AcademicRecommendation pendingRecommendation = mock(AcademicRecommendation.class);
    when(review.getRecommendationAcademicUnitId()).thenReturn(academicUnitId);
    when(pendingRecommendation.getReviewStatus())
        .thenReturn(AcademicRecommendationReviewStatus.PENDING);
  ...
  private List<String> availableActions(
    ...
    return (List<String>)
        ReflectionTestUtils.invokeMethod(
            service, "availableActions", application, review, recommendation, null, profile);
  }

  @SuppressWarnings("unchecked")
  private List<String> caseBlockers(Application application) {
    return (List<String>)
        ReflectionTestUtils.invokeMethod(service, "caseBlockers", application, null);
  }

  private CoreCurrentUserProfile admissionsOfficerProfile() {
    return new CoreCurrentUserProfile(
        activeUser(), List.of(), List.of(), List.of("ADMISSIONS_DECISION_MAKE"), true);
  }
