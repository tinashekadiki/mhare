...
public class ApplicationType extends AuditableEntity {
    ...
    public String getFeePolicyStatus() { return feePolicyStatus; }
    public String getFeeFreeReason() { return feeFreeReason; }
    public UUID getFeePolicyDecidedByUserId() { return feePolicyDecidedByUserId; }
    public Instant getFeePolicyDecidedAt() { return feePolicyDecidedAt; }
    ...
    public void recordFeeFreeDecision(UUID actorUserId, String reason, Instant decidedAt) {
        ...
        if (normalizedReason.length() < 10) {
            throw new IllegalArgumentException("Fee-free decision reason must contain at least 10 characters.");
        }
        ...
        feePolicyDecidedByUserId = java.util.Objects.requireNonNull(actorUserId, "Fee decision actor is required.");
        feePolicyDecidedAt = java.util.Objects.requireNonNull(decidedAt, "Fee decision time is required.");
    }

    public void setActive(boolean active, long expectedVersion) {
        if (getVersion() != expectedVersion) {
            throw new IllegalStateException("Application type was changed by another user. Refresh before retrying.");
        }
        this.active = active;
    }
