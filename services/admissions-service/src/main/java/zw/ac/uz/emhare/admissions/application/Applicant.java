...
public class Applicant extends AuditableEntity {
    ...
    public Applicant(UUID userId, String applicantNumber, String applicantCategoryCode, String firstName, String lastName, String primaryEmail) {
        this.userId = userId;
    ...
    public void correctProfile(UpdateApplicantProfileCommand command) {
        if (getVersion() != command.expectedVersion()) {
            throw new IllegalStateException("Applicant profile was changed by another user. Refresh before retrying.");
        }
        applicantCategoryCode = ApplicantCategoryCode.from(command.applicantCategoryCode()).name();
        titleCode = optionalCode(command.titleCode());
        firstName = requiredText(command.firstName(), "First name");
        middleNames = optionalText(command.middleNames());
        lastName = requiredText(command.lastName(), "Last name");
        dateOfBirth = command.dateOfBirth();
        genderCode = optionalCode(command.genderCode());
        maritalStatusCode = optionalCode(command.maritalStatusCode());
        nationalIdNumber = optionalText(command.nationalIdNumber());
        passportNumber = optionalText(command.passportNumber());
        countryId = command.countryId();
        nationalityCountryId = command.nationalityCountryId();
        placeOfBirth = optionalText(command.placeOfBirth());
        disabilityStatusCode = optionalCode(command.disabilityStatusCode());
        specialNeeds = optionalText(command.specialNeeds());
        sponsorTypeCode = optionalCode(command.sponsorTypeCode());
        primaryEmail = requiredText(command.primaryEmail(), "Primary email").toLowerCase(Locale.ROOT);
        primaryPhone = optionalText(command.primaryPhone());
        postalAddress = optionalText(command.postalAddress());
        residentialAddress = optionalText(command.residentialAddress());
    }
