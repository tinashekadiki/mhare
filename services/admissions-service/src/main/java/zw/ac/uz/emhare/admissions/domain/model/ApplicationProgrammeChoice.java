...
@Table(
    ...
    uniqueConstraints = {
      @UniqueConstraint(
          name = "uk_application_choice_rank",
          columnNames = {"application_id", "choice_rank"}),
      @UniqueConstraint(
          name = "uk_application_choice_programme",
          columnNames = {"application_id", "programme_id"})
    })
public class ApplicationProgrammeChoice extends AuditableEntity {
  ...
  public ApplicationProgrammeChoice(
      Application application, ProgrammeSelectionSnapshot programmeSelection, int choiceRank) {
  ...
  public void enterAcademicReview() {
    if (choiceStatus != ProgrammeChoiceStatus.ELIGIBLE
      ...
      throw new IllegalStateException(
          "Only an eligible programme choice can enter academic review.");
    }
  ...
  public void recordDecision(DecisionOutcome decision, String reason) {
    if (choiceStatus != ProgrammeChoiceStatus.UNDER_ACADEMIC_REVIEW) {
      throw new IllegalStateException(
          "Only a programme choice under academic review can receive an admission decision.");
    }
    choiceStatus =
        decision == DecisionOutcome.ADMIT
            ? ProgrammeChoiceStatus.ADMITTED
  ...
  public void recordOfferResponse(OfferResponseType response, String reason) {
    if (choiceStatus != ProgrammeChoiceStatus.OFFERED) {
      throw new IllegalStateException(
          "Only an offered programme choice can record an offer response.");
    }
  ...
  public void reopenAfterOfferClosed(String reason) {
    if (choiceStatus != ProgrammeChoiceStatus.OFFERED) {
      throw new IllegalStateException(
          "Only an offered programme choice can return to admitted status.");
    }
