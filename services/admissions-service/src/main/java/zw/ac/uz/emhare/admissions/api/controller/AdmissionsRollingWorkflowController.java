...
public class AdmissionsRollingWorkflowController {
  ...
  public AdmissionsRollingWorkflowController(
      AdmissionsWorkItemService workItemService,
      AdmissionsRollingWorkflowService workflowService,
  ...
  @PreAuthorize(
      "@admissionsRbac.has(authentication, 'ADMISSIONS_APPLICATION_REVIEW') or @admissionsRbac.has(authentication, 'ADMISSIONS_ACADEMIC_UNIT_RECOMMEND')")
  ...
  public WorkItemCase recalculate(
      Authentication authentication, @PathVariable("applicationId") UUID applicationId) {
  ...
  @PreAuthorize("@admissionsRbac.has(authentication, 'ADMISSIONS_ELIGIBILITY_REVIEW')")
  public WorkItemCase resolve(
      Authentication authentication,
      @PathVariable("applicationId") UUID applicationId,
      @PathVariable("choiceId") UUID choiceId,
      @Valid @RequestBody EligibilityResolutionRequest request) {
    var profile = coreIdentityClient.syncCurrentUser(authentication);
    workflowService.resolveEligibility(
        applicationId, choiceId, request.outcome(), request.reason(), profile.user().id());
    return workItemService.get(applicationId, profile);
  }
  ...
  public WorkItemCase recommend(
      Authentication authentication,
      @PathVariable("applicationId") UUID applicationId,
      @PathVariable("choiceId") UUID choiceId,
      @Valid @RequestBody AcademicRecommendationRequest request) {
    var profile = coreIdentityClient.syncCurrentUser(authentication);
    workflowService.recommend(
        applicationId, choiceId, request.recommendation(), request.reason(), profile);
    return workItemService.get(applicationId, profile);
  }
  ...
  public WorkItemCase returnRecommendation(
    ...
    var profile = coreIdentityClient.syncCurrentUser(authentication);
    workflowService.returnRecommendation(
        applicationId, choiceId, request.reason(), profile.user().id());
    return workItemService.get(applicationId, profile);
  }

  @PostMapping("/applications/{applicationId}/choices/{choiceId}/decision")
  @PreAuthorize("@admissionsRbac.has(authentication, 'ADMISSIONS_DECISION_MAKE')")
  public AdmissionOfferSummary decide(
      Authentication authentication,
      @PathVariable("applicationId") UUID applicationId,
      @PathVariable("choiceId") UUID choiceId,
      @Valid @RequestBody AdmissionDecisionRequest request) {
    var profile = coreIdentityClient.syncCurrentUser(authentication);
    return workflowService.decide(
        applicationId, choiceId, request.decision(), request.reason(), profile.user().id());
  }

  @PutMapping("/offers/{offerId}")
  @PreAuthorize("@admissionsRbac.has(authentication, 'ADMISSIONS_OFFER_MANAGE')")
  public AdmissionOfferSummary updateOffer(
      Authentication authentication,
      @PathVariable("offerId") UUID offerId,
      @Valid @RequestBody UpdateOfferRequest request) {
    return offerService.update(
        offerId, request.offerType(), request.conditionsText(), authorization(authentication));
  }

  @PostMapping("/offers/{offerId}/document-generation")
  @PreAuthorize("@admissionsRbac.has(authentication, 'ADMISSIONS_OFFER_MANAGE')")
  public DocumentGenerationResult generateDocument(
      Authentication authentication, @PathVariable("offerId") UUID offerId) {
    var profile = coreIdentityClient.syncCurrentUser(authentication);
  ...
  @PreAuthorize("@admissionsRbac.has(authentication, 'ADMISSIONS_OFFER_MANAGE')")
  public PublicationResult publishAndSend(
      Authentication authentication, @PathVariable("offerId") UUID offerId) {
    var profile = coreIdentityClient.syncCurrentUser(authentication);
  ...
  private String authorization(Authentication authentication) {
    ...
    throw new IllegalStateException(
        "JWT authentication is required for governed offer-letter pricing.");
  }

  @PostMapping("/offers/{offerId}/email-retry")
  @PreAuthorize("@admissionsRbac.has(authentication, 'ADMISSIONS_OFFER_MANAGE')")
  public PublicationResult retryEmail(
      Authentication authentication,
      @PathVariable("offerId") UUID offerId,
  ...
  public PublishedDocumentAccess applicantPublishedDocument(
      Authentication authentication, @PathVariable("offerId") UUID offerId) {
    var profile = coreIdentityClient.syncCurrentUser(authentication);
