...
public class CoreIdentityClient {
    ...
    try {
      ...
      if (exception.getStatusCode().is4xxClientError()) {
        throw new AccessDeniedException(
            "Core Identity rejected the current user context.", exception);
      }
    ...
    try {
      ...
      if (profile == null) {
        throw new ServiceDependencyUnavailableException(
            "Core Identity returned an empty institution profile.", null);
      }
      return profile;
  ...
  public OfficialNameSynchronizationResponse synchronizeOfficialName(
      String authorization, UUID userId, OfficialNameSynchronizationRequest request) {
    try {
      OfficialNameSynchronizationResponse response =
          coreIdentityHttpService.synchronizeOfficialName(authorization, userId, request);
      if (response == null) {
        throw new ServiceDependencyUnavailableException(
            "Core Identity returned no official-name synchronization result.", null);
      }
      return response;
    } catch (ServiceDependencyUnavailableException exception) {
      throw exception;
  ...
  public record CoreCurrentUserProfile(
    ...
    public CoreCurrentUserProfile(
        CoreUserSummary user, List<CoreRoleAssignmentSummary> roleAssignments) {
  ...
  public record CoreRoleAssignmentSummary(
      UUID id, UUID roleId, String roleCode, String roleName, UUID academicUnitId) {}

  public record OfficialNameSynchronizationRequest(
  ...
  public record OfficialNameSynchronizationResponse(
      UUID id,
      UUID sourceRequestId,
      UUID userId,
      String firstName,
      String middleNames,
      String lastName,
      java.time.Instant synchronizedAt) {}
