...
import zw.ac.uz.emhare.admissions.application.command.CreateAdmissionOfferCommand;
...
import zw.ac.uz.emhare.admissions.application.command.RecordEvaluationCommand;
...
public class AdmissionsSelectionOfferController {
    ...
    public AdmissionRequirementSetSummary createRequirementSet(
        ...
        return workflowService.createRequirementSet(
                ...
                request.qualificationGroups() == null ? java.util.List.of() : request.qualificationGroups().stream()
                        .map(group -> new AdmissionsSelectionOfferService.QualificationRequirementGroupInput(
                                group.code(), group.name(), group.minimumSatisfiedItems(), group.sortOrder(),
                                group.items().stream().map(item -> new AdmissionsSelectionOfferService.QualificationRequirementItemInput(
                                        item.qualificationLevel(), item.minimumCount(), item.minimumTotalPoints(),
                                        item.minimumDurationMonths(), item.sortOrder())).toList()))
                        .toList());
    }
