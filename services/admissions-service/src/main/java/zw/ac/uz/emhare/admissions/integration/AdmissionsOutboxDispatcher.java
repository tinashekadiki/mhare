...
/** @author Tinashe K */
@Component
public class AdmissionsOutboxDispatcher {
    ...
    public AdmissionsOutboxDispatcher(
    ...
    public void dispatchPendingEvents() {
        ...
        List<AdmissionsOutboxEvent> events = outboxEventRepository.lockNextDispatchBatch(dispatchStartedAt);
        for (AdmissionsOutboxEvent event : events) {
            try {
    ...
    private void publishWithBrokerConfirmation(AdmissionsOutboxEvent event) {
        Message message = MessageBuilder
                .withBody(event.getPayload().getBytes(StandardCharsets.UTF_8))
                .setContentType("application/json")
                .setMessageId(event.getId().toString())
                .setType(event.getEventType())
                .setHeader("source-service", "admissions-service")
                .build();
