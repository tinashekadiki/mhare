...
public class QualificationReferenceDataService {
  ...
  private static final Set<String> SUBJECT_GROUP_CODES =
      Set.of("ARTS", "COMMERCIAL", "ENGLISH", "HUMANITIES", "MATHEMATICS", "SCIENCE", "TECHNICAL");
  ...
  public QualificationReferenceData activeReferenceData() {
    return new QualificationReferenceData(
        examBodyRepository.findAllByActiveTrueAndDeletedAtIsNullOrderByNameAsc().stream()
            .map(
                value -> new ReferenceOption(value.getId(), value.getCode(), value.getName(), null))
            .toList(),
  ...
  public GradeReferenceOption createGrade(
      String levelCode, String grade, BigDecimal points, boolean pass, int sortOrder) {
    ...
    GradingScaleValue value =
        new GradingScaleValue(scale, normalizedGrade, points, pass, sortOrder);
  ...
  private List<SubjectReferenceOption> activeSubjectOptions(SubjectLevel level) {
    return subjectRepository
        .findAllByLevelAndActiveTrueAndDeletedAtIsNullOrderByNameAsc(level)
        .stream()
  ...
  private List<GradeReferenceOption> gradeOptions(QualificationLevel level) {
    return gradingScaleRepository
        .findApplicableScale(level, LocalDate.now(clock))
        .map(
            scale ->
                gradingScaleValueRepository
                    .findAllByGradingScaleIdAndDeletedAtIsNullOrderBySortOrderAsc(scale.getId())
                    .stream()
  ...
  private SubjectReferenceOption subjectOption(AdmissionSubject subject) {
    return new SubjectReferenceOption(
        ...
        subject.getCode(),
        subject.getName(),
        subject.getSubjectGroupCode(),
        subject.isScienceSubject(),
        subject.isMathematicsSubject(),
        subject.isEnglishSubject(),
        subject.isActive(),
        subject.getVersion());
  }
        ...
        .orElseThrow(
            () ->
                new IllegalStateException(
                    "No effective grading scale is configured for "
                        + level.name().replace('_', ' ')
                        + "."));
  }

  private void assertSubjectCodeAvailable(
      SubjectLevel level, String code, AdmissionSubject currentSubject) {
    ...
    if (subjectRepository.existsByLevelAndCodeIgnoreCaseAndDeletedAtIsNull(level, code)) {
      throw new IllegalArgumentException(
          "A subject with this code already exists at the selected level.");
    }
  }

  private void assertGradeAvailable(
      GradingScale scale, String grade, GradingScaleValue currentValue) {
    ...
    if (gradingScaleValueRepository
        ...
        .isPresent()) {
      throw new IllegalArgumentException(
          "This grade already exists in the selected grading scale.");
    }
  }
  ...
  private QualificationLevel qualificationLevel(String levelCode) {
    try {
      QualificationLevel level =
          QualificationLevel.valueOf(requiredUppercase(levelCode, "Grade level", 30));
  ...
  private void assertVersion(long actualVersion, long expectedVersion, String recordName) {
    if (actualVersion != expectedVersion) {
      throw new IllegalStateException(
          recordName + " was changed by another user. Refresh and try again.");
    }
  }
}
