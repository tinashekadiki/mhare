...
@Table(
    ...
    uniqueConstraints = @UniqueConstraint(name = "uk_admission_cycles_code", columnNames = "code"))
public class AdmissionCycle extends AuditableEntity {
  ...
  public void open(Instant instant) {
    ...
    if (instant.isBefore(opensAt) || instant.isAfter(closesAt)) {
      throw new IllegalStateException(
          "Admission cycle cannot be opened outside its configured date range.");
    }
    status = AdmissionCycleStatus.OPEN;
  }
  ...
  public void closeApplications() {
    if (status != AdmissionCycleStatus.OPEN) {
      throw new IllegalStateException(
          "Only an open admission cycle can be closed for applications.");
    }
  ...
  public void beginSelection() {
    if (status != AdmissionCycleStatus.CLOSED) {
      throw new IllegalStateException(
          "Admissions selection can only begin after applications close.");
    }
