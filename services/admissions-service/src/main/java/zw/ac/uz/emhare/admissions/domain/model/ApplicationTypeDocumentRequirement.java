...
public class ApplicationTypeDocumentRequirement extends AuditableEntity {
    ...
    public ApplicationTypeDocumentRequirement(
            ...
            boolean required,
            int sortOrder) {
        if (sortOrder <= 0) throw new IllegalArgumentException("Document requirement sort order must be positive.");
        this.applicationType = applicationType;
        this.requirementCode = requireText(requirementCode, "Requirement code").toUpperCase(Locale.ROOT);
    ...
    public void configure(String requirementName, boolean required, int sortOrder) {
        if (sortOrder < 1) throw new IllegalArgumentException("Document requirement sort order must be positive.");
        this.requirementName = requireText(requirementName, "Requirement name");
        this.required = required;
        this.sortOrder = sortOrder;
        this.active = true;
