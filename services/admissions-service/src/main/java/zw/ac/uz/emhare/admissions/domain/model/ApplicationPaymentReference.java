...
@Audited
@Entity
@Table(name = "application_payment_references", uniqueConstraints = @UniqueConstraint(name = "uk_application_payment_references_reference", columnNames = "reference"))
public class ApplicationPaymentReference extends AuditableEntity {
    ...
    public boolean synchronize(ApplicationPaymentReferenceUpdatedEvent financePaymentReference) {
        if (!application.getId().equals(financePaymentReference.applicationId())) {
            throw new IllegalArgumentException("Finance payment reference belongs to a different application.");
        }
        ...
        if (financePaymentReferenceId != null
            ...
            throw new IllegalStateException("Finance changed the immutable payment reference identifier.");
        }
