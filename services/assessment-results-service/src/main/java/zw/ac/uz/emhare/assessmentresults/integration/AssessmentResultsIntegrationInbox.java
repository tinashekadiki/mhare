...
/** @author Tinashe K */
@Component
public class AssessmentResultsIntegrationInbox {
    private final JdbcTemplate jdbcTemplate;

    public AssessmentResultsIntegrationInbox(JdbcTemplate jdbcTemplate) {
    ...
    public void markProcessed(UUID eventId, Instant processedAt) {
        int updated = jdbcTemplate.update("""
                UPDATE integration_inbox SET processed_at = ?
                WHERE event_id = ? AND processed_at IS NULL
                """, Timestamp.from(processedAt), eventId);
        if (updated != 1) throw new IllegalStateException("Claimed registration event could not be marked processed.");
    }
}
