...
public class FinanceStudentDiscountController {
    ...
    @PostMapping public DiscountSummary create(Authentication authentication,
            @Valid @RequestBody CreateDiscount request) { return service.create(request, actor(authentication)); }
    ...
    public DiscountSummary move(Authentication authentication, @PathVariable("discountId") UUID discountId,
            @PathVariable("action") String action, @Valid @RequestBody DiscountDecision request) {
        return service.move(discountId, action, request, actor(authentication));
    }
