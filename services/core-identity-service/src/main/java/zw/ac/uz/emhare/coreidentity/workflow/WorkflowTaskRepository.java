...
public interface WorkflowTaskRepository extends JpaRepository<WorkflowTask, UUID> {
    ...
    @Query("""
            ...
            where task.status in :statuses
              and (
                (task.assigneeType = zw.ac.uz.emhare.coreidentity.workflow.WorkflowAssigneeType.USER
                  and task.assignedUser.id = :userId)
                or
                (task.assigneeType = zw.ac.uz.emhare.coreidentity.workflow.WorkflowAssigneeType.ROLE
                  and exists (
                    ...
                    where assignment.user.id = :userId
                      ...
                      and (
                        task.scopeType = zw.ac.uz.emhare.coreidentity.workflow.WorkflowScopeType.INSTITUTION
