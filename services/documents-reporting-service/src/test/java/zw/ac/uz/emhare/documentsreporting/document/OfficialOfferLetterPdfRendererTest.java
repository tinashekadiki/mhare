...
class OfficialOfferLetterPdfRendererTest {
  ...
  void rendersGovernedOfferLetterWithVerificationMetadata() throws Exception {
    ...
    when(offer.getContentSnapshot())
        .thenReturn(
            new OfferLetterContentSnapshot(
                ...
                "admissions@uz.ac.zw",
                "www.uz.ac.zw",
                "14 Samora Machel Avenue, Harare",
                "LOCAL",
                "UNDERGRAD",
                "Undergraduate",
                "March 2028 Intake",
                ...
                List.of(
                    ...
                    new BankAccountSnapshot(
                        ...
                        null,
                        "After accepting this offer, eMhare will generate your registration number. Quote that registration number as the payment reference.")),
                "42b272b2-6f22-4fad-baf2-cfc3d6438e76",
                "Dr Registrar",
                "Registrar",
                "UZ-OFFER-LETTER-2026-01"));

    var rendered =
        new OfficialOfferLetterPdfRenderer(
                documentId ->
                    OfficialOfferLetterPdfRendererTest.class.getResourceAsStream(
                        "/documents/uz-logo.jpg"))
            .render(document, offer);
    ...
    try (var pdf = Loader.loadPDF(rendered.content())) {
      ...
      assertTrue(
          java.util.stream.StreamSupport.stream(
                      pdf.getPage(0).getResources().getXObjectNames().spliterator(), false)
                  .count()
              >= 3,
          "The institution logo and signature must remain primary while the eMhare watermark is present.");
      String text = new PDFTextStripper().getText(pdf);
      assertTrue(text.contains("UNIVERSITY OF ZIMBABWE"));
      assertTrue(
          text.contains(
              "ADMISSION IN THE YEAR 2028 TO THE Bachelor of Science in Computer Science (BSC-CS) IN THE Faculty of Science"));
  ...
  void rendersInternationalConditionalOfferWithoutInventingFeeAmounts() throws Exception {
    ...
    when(offer.getConditionsText())
        .thenReturn("Submit the final degree transcript before registration.");
    when(offer.getAcceptanceDeadline()).thenReturn(Instant.parse("2028-02-28T21:59:59Z"));
    when(offer.getCommencementDate()).thenReturn(LocalDate.parse("2028-03-04"));
    when(offer.getContentSnapshot())
        .thenReturn(
            new OfferLetterContentSnapshot(
                ...
                "2028.1",
                List.of(),
                List.of("Passport", "Original degree certificate"),
                new FeeScheduleSnapshot(
                    UUID.randomUUID(),
                    1,
                    "INTL-MBA",
    ...
    try (var pdf = Loader.loadPDF(rendered.content())) {
      ...
      assertTrue(
          text.contains("Finance has not supplied a published fee snapshot for this letter."));
    ...
    when(offer.getContentSnapshot())
        .thenReturn(
            new OfferLetterContentSnapshot(
                ...
                "2029.1",
                List.of(),
                List.of("National identity document"),
                new FeeScheduleSnapshot(
                    UUID.randomUUID(),
                    2,
                    "UG-BACC-2029",
                    ...
                    List.of(
                        new FeeLineSnapshot(
                            null, "Application charge", new BigDecimal("10.00"), null),
                        new FeeLineSnapshot(
                            "ACCEPTANCE_DEPOSIT",
                            "Acceptance deposit",
