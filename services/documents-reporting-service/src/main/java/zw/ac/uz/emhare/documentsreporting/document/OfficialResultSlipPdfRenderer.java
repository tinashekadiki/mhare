...
public class OfficialResultSlipPdfRenderer {
  ...
  private static final DateTimeFormatter DATE_FORMAT =
      DateTimeFormatter.ofPattern("dd MMM uuuu HH:mm 'CAT'").withZone(ZoneId.of("Africa/Harare"));
  ...
  public RenderedPdf render(
    ...
    if (results.isEmpty()) {
      throw new IllegalStateException(
          "An official result slip requires at least one published Module result.");
    }
    try (PDDocument pdf = new PDDocument();
      ...
      int pageCount =
          Math.max(1, (results.size() + MAXIMUM_ROWS_PER_PAGE - 1) / MAXIMUM_ROWS_PER_PAGE);
  ...
  private void renderPage(
    ...
    try (PDPageContentStream content = new PDPageContentStream(pdf, page)) {
      ...
      text(
          content, REGULAR, 10, 405, y - 18, Integer.toString(decision.getProgrammePeriodNumber()));
      ...
      if (pageNumber == pageCount) {
        ...
        text(content, BOLD, 13, PAGE_MARGIN + 12, y - 18, limited(decision.getDecisionLabel(), 45));
        text(
            content,
            REGULAR,
            9,
        ...
        text(
            ...
            y - 16,
            "Credits passed  "
                + decision.getPassedCredits().toPlainString()
                + " / "
        ...
        text(
            ...
            y - 76,
            "Published "
                + DATE_FORMAT.format(decision.getPublishedAt())
                + "  |  Decision "
                + limited(decision.getDecisionNumber(), 38));
      }
  ...
  private void text(
      PDPageContentStream content, PDType1Font font, float fontSize, float x, float y, String value)
      throws IOException {
    content.beginText();
