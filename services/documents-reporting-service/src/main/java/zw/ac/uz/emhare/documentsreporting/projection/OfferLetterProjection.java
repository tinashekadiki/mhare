...
public class OfferLetterProjection extends AuditableEntity {
    ...
    public OfferLetterProjection(OfferLetterRequestedEvent event) {
        sourceEventId=event.eventId(); offerId=event.offerId(); offerVersion=event.offerVersion();
        offerNumber=event.offerNumber(); applicationId=event.applicationId(); applicationNumber=event.applicationNumber();
        applicantNumber=event.applicantNumber(); applicantName=event.applicantName(); applicantEmail=event.applicantEmail();
        programmeId=event.programmeId(); programmeCode=event.programmeCode(); programmeName=event.programmeName();
