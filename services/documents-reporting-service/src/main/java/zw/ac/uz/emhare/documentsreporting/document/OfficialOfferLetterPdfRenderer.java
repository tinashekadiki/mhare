...
public class OfficialOfferLetterPdfRenderer {
  ...
  public OfficialResultSlipPdfRenderer.RenderedPdf render(
      GeneratedDocument document, OfferLetterProjection offer) {
    try {
      OfferLetterContentSnapshot snapshot = content(offer);
      JasperPrint print =
          JasperFillManager.fillReport(
              report, parameters(document, offer, snapshot), new JREmptyDataSource(1));
      ...
      throw new IllegalStateException(
          "Official offer-letter PDF could not be rendered.", exception);
    }
  }

  private Map<String, Object> parameters(
      GeneratedDocument document,
      OfferLetterProjection offer,
    ...
    values.put(
        "INSTITUTION_TITLE",
        first(snapshot.institutionName(), "University of Zimbabwe").toUpperCase(Locale.ROOT));
    values.put(
        "ISSUE_DATE",
        document.getRequestedAt() == null ? "" : INSTANT_DATE.format(document.getRequestedAt()));
    ...
    values.put(
        "OPENING_PARAGRAPH",
        "I am pleased to inform you that a place has been reserved for you on the above mentioned degree programme.");
    ...
    values.put(
        "EQUALITY_TERM",
        "The University is a non-discriminatory and inclusive equal opportunity institution and students are expected to attend all official learning schedules and activities as prescribed. The University academic business runs 24 hours 7 days a week during a defined semester.");
    values.put(
        "AMENDMENT_TERM",
        "Kindly note that this offer is made without prejudice to the rights which the University may have to amend, withdraw or cancel in the event of you or the University being unable to meet the conditions of the offer.");
    values.put(
        "REGISTRATION_TERM",
        "Admittance to the University is made subject to your accepting the conditions set in this letter and registering in the stipulated period. Failure to do so will lead to the University withdrawing your name from its list of successful applicants for the "
            + academicYear(document, offer)
            + " Academic year.");
    values.put(
        "BLENDED_LEARNING_TERM",
        "The University of Zimbabwe offers blended learning and each student is required to bring their own Laptop/Tablet to support their eLearning activities.");
    values.put(
        "CONGRATULATIONS",
        "Congratulations on being accepted to the University of Zimbabwe. We wish you all the best in your academic journey.");
    values.put(
        "SIGNATORY",
        first(snapshot.signatoryName(), "Registrar")
            + "\n"
    ...
    values.put(
        ...
        "B. I accept/do not accept the university offer as outlined in this letter. By accepting this offer I confirm that the programme offered and my personal details are correct.");
  ...
  private OfferLetterContentSnapshot content(OfferLetterProjection offer) {
    ...
    return new OfferLetterContentSnapshot(
  ...
  private InputStream logo() {
    InputStream logo =
        OfficialOfferLetterPdfRenderer.class.getResourceAsStream("/documents/uz-logo.jpg");
    if (logo == null)
      throw new IllegalStateException("The University of Zimbabwe offer-letter logo is missing.");
    return logo;
  }
  ...
  private String institutionAddress(OfferLetterContentSnapshot snapshot) {
    String address =
        first(
            snapshot.institutionPostalAddress(),
  ...
  private String applicantAddress(
      OfferLetterProjection offer, OfferLetterContentSnapshot snapshot) {
    return first(offer.getApplicantName(), "Applicant")
        + " ("
        + first(offer.getApplicationNumber(), "-")
        + ")\n"
        + first(snapshot.applicantPostalAddress(), offer.getApplicantEmail());
  }

  private String admissionHeading(
      GeneratedDocument document,
      OfferLetterProjection offer,
      OfferLetterContentSnapshot snapshot) {
    String academicUnit =
        snapshot.academicUnitName() == null || snapshot.academicUnitName().isBlank()
            ? ""
            : " IN THE " + snapshot.academicUnitName().trim();
    return "ADMISSION IN THE YEAR "
        + academicYear(document, offer)
        + " TO THE "
        + first(offer.getProgrammeName(), "PROGRAMME")
        + " ("
        + first(offer.getProgrammeCode(), "-")
        + ")"
        + academicUnit;
  }
  ...
  private String registrationInstruction(
      OfferLetterProjection offer, OfferLetterContentSnapshot snapshot) {
    String registration =
        offer.getRegistrationDate() == null
    ...
    String evidence =
        snapshot.requiredVerificationDocuments().isEmpty()
            ? "your original identity and academic evidence"
            : "the originals of " + joined(snapshot.requiredVerificationDocuments());
    String conditions =
        offer.getConditionsText() == null || offer.getConditionsText().isBlank()
            ? ""
            : " Conditions of this offer: " + offer.getConditionsText().trim();
    return registration
        + " Use your application number when completing online registration through your authenticated eMhare workspace. You are also expected to bring "
        + evidence
        + " for verification at the Faculty Office."
        + conditions;
  }
  ...
  private String feeIntroduction(FeeScheduleSnapshot schedule) {
    ...
    return "The fees payable per semester are as indicated below. The fees are payable in "
        + schedule.transactionCurrencyCode()
        + ". PLEASE NOTE THAT FEES ARE SUBJECT TO CHANGE WITHOUT NOTICE.";
  }

  private FeeColumns feeColumns(FeeScheduleSnapshot schedule) {
    List<FeeLineSnapshot> lines =
        schedule == null || schedule.lines().isEmpty() ? legacyFeeHeadings() : schedule.lines();
    ...
    for (int column = 0; column < 3; column++) {
      ...
      items.add(
          lines.subList(start, end).stream()
              .map(line -> first(line.description(), line.code()))
      ...
      amounts.add(
          lines.subList(start, end).stream()
              .map(
                  line ->
                      schedule == null || schedule.lines().isEmpty()
                          ? "-"
  ...
  private String paymentInstruction(OfferLetterContentSnapshot snapshot) {
    ...
    String rating =
        schedule != null && schedule.exchangeRateId() != null
    ...
    if (!snapshot.bankAccounts().isEmpty()) {
      ...
      for (BankAccountSnapshot account : snapshot.bankAccounts()) {
        accountsByBank
            .computeIfAbsent(account.bankName().trim(), ignored -> new java.util.ArrayList<>())
            .add(account);
      }
      String configuredAccounts =
          accountsByBank.entrySet().stream()
              .map(
                  entry ->
                      entry.getKey()
                          + ": "
                          + entry.getValue().stream()
                              .map(
                                  account ->
                                      account.currencyCode() + " " + account.accountNumber().trim())
                              .reduce((left, right) -> left + " | " + right)
                              .orElse(""))
              .reduce((left, right) -> left + "\n" + right)
              .orElse("");
      String references =
          snapshot.bankAccounts().stream()
    ...
    if (snapshot.bankDetails() == null
        || isBlank(snapshot.bankDetails().bankName())
        || isBlank(snapshot.bankDetails().accountNumber())) {
      return "Bank details have not been configured in the Institution Profile. Use only payment methods and references displayed in your authenticated eMhare Finance workspace."
          + rating;
    }
    ...
    String details =
        "BANK: "
            + bank.bankName().trim()
            + " | CURRENCY: NOT RECORDED | ACCOUNT NUMBER: "
  ...
  private String commencementInstruction(OfferLetterProjection offer) {
    ...
    return "Lectures commence on "
        + date(offer.getCommencementDate())
        ...
        schedule == null
            ...
            : schedule.lines().stream()
                .filter(
                    line ->
                        line.code() != null
                            && line.code().toUpperCase(Locale.ROOT).contains("DEPOSIT"))
                .findFirst()
                .map(
                    line ->
  ...
  private int academicYear(GeneratedDocument document, OfferLetterProjection offer) {
    ...
    if (document.getRequestedAt() != null)
      return document.getRequestedAt().atZone(ZoneId.of("Africa/Harare")).getYear();
    return LocalDate.now(ZoneId.of("Africa/Harare")).getYear();
  }

  private byte[] addMetadata(
      byte[] generatedPdf, GeneratedDocument document, OfferLetterProjection offer)
      throws Exception {
    try (PDDocument pdf = Loader.loadPDF(generatedPdf);
      ...
      EmhareDocumentBranding.addProductWatermarks(pdf);
      pdf.save(output);
