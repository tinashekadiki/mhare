...
function isFrontendProductionSource(filePath) {
  ...
  return !/(^|\/)(node_modules|\.nuxt|\.output|coverage|dist)(\/|$)/.test(filePath)
...
export function findMissingBackendCoverageModules(changedLinesByFile, coveredModulePaths) {
  const changedModulePaths = new Set(
    [...changedLinesByFile.keys()]
      .filter(isBackendProductionSource)
      .map(backendModulePath)
  )
  return [...changedModulePaths]
    .filter(modulePath => !coveredModulePaths.has(modulePath))
    .sort()
}
...
export function parseUnifiedDiff(diffText) {
  ...
  for (const line of diffText.split(/\r?\n/)) {
    ...
    if (!currentFile || !isProductionSource(currentFile) || !line.startsWith('@@')) continue
    const hunk = line.match(/\+(\d+)(?:,(\d+))?\s/)
    if (!hunk) continue
    const firstChangedLine = Number(hunk[1])
    const changedLineCount = hunk[2] === undefined ? 1 : Number(hunk[2])
    const changedLines = changedLinesByFile.get(currentFile)
    for (let offset = 0; offset < changedLineCount; offset += 1) {
      changedLines.add(firstChangedLine + offset)
    }
  }
...
export function collectChangedLines(repositoryRoot, baseReference) {
  ...
  const changedLinesByFile = parseUnifiedDiff(diffText)
  const untrackedFiles = runGit(repositoryRoot, [
    'ls-files',
    '--others',
    '--exclude-standard'
  ]).split(/\r?\n/).filter(Boolean)
...
export function parseJacocoXml(xmlText, modulePath) {
  ...
  for (const packageMatch of xmlText.matchAll(/<package\s+name="([^"]*)"[^>]*>([\s\S]*?)<\/package>/g)) {
    const packageName = decodeXml(packageMatch[1])
    const packageBody = packageMatch[2]
    for (const sourceMatch of packageBody.matchAll(/<sourcefile\s+name="([^"]+)"[^>]*>([\s\S]*?)<\/sourcefile>/g)) {
...
export function evaluateChangedCoverage(changedLinesByFile, coverageByFile, threshold = REQUIRED_COVERAGE_PERCENTAGE) {
  const totals = {
    linesCovered: 0,
    linesTotal: 0,
    branchesCovered: 0,
  ...
  for (const [filePath, changedLines] of [...changedLinesByFile].sort(([left], [right]) => left.localeCompare(right))) {
  ...
  return {
    passed: (linePercentage === null || linePercentage >= threshold)
...
function loadCoverage(repositoryRoot, changedLinesByFile) {
  ...
  if (hasBackendChanges) {
    const jacocoReports = [
      ...findFiles(resolve(repositoryRoot, 'services'), filePath => filePath.endsWith('/target/site/jacoco/jacoco.xml')),
      ...findFiles(resolve(repositoryRoot, 'libraries'), filePath => filePath.endsWith('/target/site/jacoco/jacoco.xml'))
    ]
    if (jacocoReports.length === 0) {
      throw new Error('No JaCoCo XML reports were found. Run mvn verify before the changed-coverage gate.')
    }
    const coveredModulePaths = new Set(jacocoReports.map(reportPath => {
    ...
    const missingModulePaths = findMissingBackendCoverageModules(changedLinesByFile, coveredModulePaths)
    if (missingModulePaths.length > 0) {
      throw new Error(`JaCoCo XML is missing for changed modules: ${missingModulePaths.join(', ')}. Add or run tests so every changed module produces coverage during mvn verify.`)
    }
  ...
  if (hasFrontendChanges) {
    ...
    if (!existsSync(lcovPath)) {
      throw new Error('Frontend LCOV output was not found. Run npm run test:unit:coverage before the changed-coverage gate.')
    }
...
function printResult(result) {
  for (const file of result.files) {
    ...
    console.log(`${file.filePath}: lines ${metricDisplay(file.linesCovered, file.linesTotal)}, branches ${metricDisplay(file.branchesCovered, file.branchesTotal)}`)
    ...
    if (file.partiallyCoveredBranchLines.length > 0) {
      console.log(`  uncovered branches on lines: ${[...new Set(file.partiallyCoveredBranchLines)].join(', ')}`)
    }
  }
  console.log(`Changed executable lines: ${metricDisplay(result.totals.linesCovered, result.totals.linesTotal)}`)
  console.log(`Changed branches: ${metricDisplay(result.totals.branchesCovered, result.totals.branchesTotal)}`)
  console.log(`Required minimum: ${result.threshold.toFixed(2)}% for lines and branches when applicable`)
}
...
export function runChangedCoverageGate(repositoryRoot = REPOSITORY_ROOT) {
  ...
  if (changedLinesByFile.size === 0) {
    console.log(`No changed backend or frontend production source was found relative to ${baseReference}.`)
    return 0
  }
