...
const { semesterItems, studyPeriodLabel, toProgrammePeriodNumber, yearOfStudyItems } =
...
const selectedEnrolment = computed(
  () =>
    activeEnrolments.value.find(
      (enrolment) => enrolment.id === registrationForm.programmeEnrolmentId,
    ) ?? null,
);
const selectedProgramme = computed(
  () =>
    academicOverview.value?.programmes.find(
      (programme) => programme.id === selectedEnrolment.value?.programmeId,
    ) ?? null,
);
const yearOfStudyOptions = computed(() =>
  yearOfStudyItems(selectedProgramme.value?.maximumDurationPeriods ?? 2),
);
const programmeEnrolmentItems = computed(() =>
  activeEnrolments.value.map((enrolment) => ({
...
const academicPeriodItems = computed(() =>
  openAcademicPeriods.value.map((period) => ({
...
const electiveItems = computed(() =>
  electiveModules.value.map((module) => ({
...
const registrationStartGuidance = computed(() => {
  ...
  if (!activeEnrolments.value.length)
    instructions.push("Registry must activate your programme enrolment before you can register.");
  if (!openAcademicPeriods.value.length)
    instructions.push("Registry must open an academic period for student registration.");
  return instructions;
});
const registrationFormGuidance = computed(() => {
  const instructions: string[] = [];
  if (!registrationForm.programmeEnrolmentId)
    instructions.push("Select your active programme enrolment.");
  if (!registrationForm.academicPeriodId) instructions.push("Select an open academic period.");
  if (loadingCatalogue.value)
    instructions.push("Wait for the curriculum Module catalogue to finish loading.");
  else if (!registrationCatalogue.value?.modules.length)
    instructions.push(
      "Registry must attach active Modules to the applicable approved curriculum version.",
    );
  return instructions;
});
...
function nextProgrammePeriodNumber(programmeEnrolmentId?: string) {
  const latestRegistration = registrations.value.find(
    (registration) =>
...
async function createRegistration() {
  ...
  registrationFormError.value = "";
  try {
    const created = await api.request<RegistrationSummary>(
      "/api/student-records/registrations/mine",
      {
        method: "POST",
        body: {
...
function statusTone(status: string) {
  ...
  if (status === "DRAFT" || status === "SUBMITTED" || status === "ACADEMIC_APPROVED")
    return "info" as const;
  if (status === "REJECTED" || status === "SUSPENDED" || status === "WITHDRAWN")
  ...
  <div class="min-h-screen bg-default">
    <header class="border-b border-muted bg-elevated/70">
      <UContainer class="flex min-h-16 items-center justify-between gap-4 py-3">
        <EmhareProductBrand label="eMhare Student" description="Academic self-service" />
    ...
    <main>
      <UContainer class="space-y-5 py-5 sm:py-7">
        ...
        <EmhareFeedbackState
          ...
          description="Retrieving the governed student record and registrations."
        />
        <EmhareFeedbackState
          v-else-if="loadError"
          state="error"
        ...
        <template v-else-if="workspace">
          ...
          <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <EmhareKpiCard
              ...
              :value="workspace.studentNumber"
              hint="Institutional identifier"
            ...
            <EmhareKpiCard
              ...
              hint="Current programme enrolments"
              icon="i-lucide-graduation-cap"
              tone="success"
            />
            <EmhareKpiCard
              label="Confirmed registrations"
              :value="registrations.filter((item) => item.status === 'CONFIRMED').length"
              hint="Approved academic periods"
              icon="i-lucide-badge-check"
              tone="success"
            />
            <EmhareKpiCard
              label="Confirmed credits"
              :value="totalConfirmedCredits.toFixed(2)"
          ...
          >
            <UCard>
              ...
              <EmhareDescriptionList
                :items="[
                  {
                    label: 'Full name',
                    value: [workspace.firstName, workspace.middleNames, workspace.lastName]
                      .filter(Boolean)
                      .join(' '),
                  },
            ...
            <UCard :ui="{ body: 'p-0' }">
              ...
              <EmharePaginatedTable
                :data="workspace.programmeEnrolments"
                :columns="enrolmentColumns"
              >
                <template #programmeCode-cell="{ row }"
                  ><span class="font-mono font-semibold text-primary">{{
                    row.original.programmeCode
                  }}</span></template
                >
                <template #commencementDate-cell="{ row }">{{
                  formatDate(row.original.commencementDate)
                }}</template>
                ...
                <template #empty
                  ><div class="py-10 text-center text-sm text-muted">
                    No programme enrolments are linked to this student.
                  </div></template
                >
              </EmharePaginatedTable>
            </UCard>
          ...
          <section v-else-if="activeSection === 'REGISTRATIONS'">
            <UCard :ui="{ body: 'p-0' }">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p class="text-sm text-muted">
                      Draft, submitted, approved, confirmed, and rejected registration sessions.
                    </p>
                  </div>
                  <EmhareGuidedActionButton
                    label="Start registration"
              ...
              <EmharePaginatedTable :data="registrations" :columns="registrationColumns" sticky>
                ...
                >
                <template #programmePeriodNumber-cell="{ row }"
                  ><span class="whitespace-nowrap">{{
                    studyPeriodLabel(row.original.programmePeriodNumber)
                  }}</span></template
                >
                <template #totalCredits-cell="{ row }">{{
                  Number(row.original.totalCredits).toFixed(2)
                }}</template>
                <template #status-cell="{ row }"
                  ><EmhareStatusPill
                    :label="row.original.status"
                ...
                <template #actions-cell="{ row }">
                  <div class="flex items-center justify-end gap-2">
                    <UButton
                      :aria-label="`View ${row.original.academicPeriodName} registration`"
                      icon="i-lucide-eye"
                      color="neutral"
                      variant="ghost"
                      @click="openRegistrationDetails(row.original)"
                    />
                    <UButton
                ...
                <template #empty
                  ><div class="py-12 text-center">
                    <UIcon name="i-lucide-clipboard-x" class="mx-auto size-8 text-muted" />
                    <p class="mt-3 font-medium text-highlighted">No registration history</p>
                    <p class="mt-1 text-sm text-muted">
                      Start registration when an academic period is open.
                    </p>
                  </div></template
                >
              </EmharePaginatedTable>
            ...
            <UCard :ui="{ body: 'p-0' }">
              <template #header>
                <div>
                  ...
                  <p class="text-sm text-muted">
                    {{
                      latestConfirmedRegistration
                        ? `${latestConfirmedRegistration.academicPeriodName} · ${studyPeriodLabel(latestConfirmedRegistration.programmePeriodNumber)}`
                        : "Modules appear after Registry confirms a registration."
                    }}
                  </p>
                </div>
              </template>
              <EmharePaginatedTable :data="currentModules" :columns="moduleColumns">
                ...
                >
                <template #curriculumModuleType-cell="{ row }"
                  ><EmhareStatusPill
                    :label="row.original.curriculumModuleType"
                    :tone="row.original.curriculumModuleType === 'COMPULSORY' ? 'info' : 'neutral'"
                /></template>
                ...
                <template #empty
                  ><div class="py-12 text-center">
                    <UIcon name="i-lucide-book-dashed" class="mx-auto size-8 text-muted" />
                    <p class="mt-3 font-medium text-highlighted">No confirmed Modules</p>
                    <p class="mt-1 text-sm text-muted">
                      Complete the registration approval workflow to populate this register.
                    </p>
                  </div></template
                >
              </EmharePaginatedTable>
  ...
  >
    <template #body>
      <form id="student-registration-form" class="space-y-5" @submit.prevent="createRegistration">
        <EmhareErrorSummary
          :errors="registrationFormError ? [registrationFormError] : []"
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <UFormField label="Programme enrolment" required class="sm:col-span-2">
            <USelect
              v-model="registrationForm.programmeEnrolmentId"
              :items="programmeEnrolmentItems"
              value-key="value"
              class="w-full"
            />
          </UFormField>
          <UFormField label="Academic period" required class="sm:col-span-2">
            <USelect
              v-model="registrationForm.academicPeriodId"
          ...
          <UFormField label="Year of study" required>
            <USelect
              v-model="registrationForm.yearOfStudy"
          ...
          <UFormField label="Semester" required>
            <USelect
              v-model="registrationForm.semesterNumber"
              :items="semesterItems"
              value-key="value"
              class="w-full"
            />
        ...
        <EmhareFeedbackState
          ...
          state="loading"
          title="Validating approved curriculum"
          description="Loading the Modules available for this study stage."
        />
        ...
        <template v-else-if="registrationCatalogue">
          ...
          <div>
            ...
            <p class="mt-1 text-xs text-muted">
              These Modules are governed by the approved curriculum and cannot be removed.
            </p>
            <div class="mt-3 space-y-2">
              <div
                v-for="module in compulsoryModules"
                :key="module.curriculumModuleId"
                class="flex items-start justify-between gap-4 rounded-md border border-muted bg-elevated/40 p-3"
              >
                <div>
                  <p class="text-sm font-medium text-highlighted">
                    <span class="font-mono text-primary">{{ module.moduleCode }}</span> ·
                    {{ module.moduleName }}
                  </p>
                  <p class="mt-1 text-xs text-muted">
                    {{ Number(module.creditValue).toFixed(2) }} credits
                  </p>
                </div>
          ...
          <div v-if="electiveItems.length">
            <UFormField
              ...
              description="Select only the electives you intend to register."
            >
              <UCheckboxGroup
                v-model="registrationForm.selectedElectiveCurriculumModuleIds"
                :items="electiveItems"
                class="mt-2"
              />
            </UFormField>
    ...
    <template #footer>
      <div class="flex w-full items-center justify-end gap-3">
        <UButton
          ...
          @click="registrationDrawerOpen = false"
        />
        <EmhareGuidedActionButton
          type="submit"
          form="student-registration-form"
          label="Create draft registration"
          icon="i-lucide-clipboard-plus"
          :loading="savingRegistration"
          guidance-title="Registration details are incomplete"
          :guidance-instructions="registrationFormGuidance"
        />
      </div>
    </template>
  </EmhareRecordDrawer>
      ...
      <template v-if="selectedRegistration">
        <EmhareDescriptionList
          :items="[
            ...
            {
              label: 'Study stage',
              value: studyPeriodLabel(selectedRegistration.programmePeriodNumber),
            },
            {
              label: 'Programme',
              value: `${selectedRegistration.programmeCode} · ${selectedRegistration.programmeName}`,
            },
        ...
        <UAlert
          ...
          title="Latest status reason"
          :description="selectedRegistration.statusReason"
        />
        <div>
    ...
    <template #footer>
      <div class="flex w-full justify-end">
        <UButton
          ...
          color="neutral"
          variant="outline"
          @click="registrationDetailsOpen = false"
        />
      </div>
    </template>
  </EmhareRecordDrawer>
