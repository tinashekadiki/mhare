...
const intakeEligibilityLocked = computed(() => Boolean(intakeForm.id && intakeForm.status !== 'DRAFT'))
...
function createIntake() {
  Object.assign(intakeForm, {
    id: null, status: 'DRAFT', academicYearId: academicYears.value[0]?.id ?? '', code: '', name: '',
    startsOn: '', endsOn: '', maximumProgrammeChoices: 3,
    programmeLevelIds: [], programmeIds: [], changeReason: '', expectedVersion: 0
  })
...
<template>
        ...
        >
          ...
          <div class="overflow-hidden rounded-md border border-muted">
            <EmharePaginatedTable :data="intakes" :columns="intakeColumns" :loading="academicSetup.loading.value">
              ...
              <template #actions-cell="{ row }">
                <div class="flex justify-end gap-1">
                  <UButton label="Edit" icon="i-lucide-pencil" color="neutral" variant="ghost" @click="editIntake(row.original)" />
                  <UButton v-if="row.original.status === 'DRAFT'" label="Open" color="primary" variant="ghost" :loading="activeCalendarActionKey === `intakes-${row.original.id}`" @click="changeCalendarRecordStatus(row.original, 'intakes', 'open')" />
                  <UButton v-else-if="row.original.status === 'OPEN'" label="Close" color="warning" variant="ghost" :loading="activeCalendarActionKey === `intakes-${row.original.id}`" @click="changeCalendarRecordStatus(row.original, 'intakes', 'close')" />
                </div>
              </template>
  ...
  <EmhareRecordDrawer v-model:open="intakeModalOpen" :title="intakeForm.id ? 'Edit intake' : 'Create intake'" description="Maintain the governed admission entry cohort, application window, and Programme eligibility.">
    <template #body>
      <form id="intake-form" class="grid gap-4 sm:grid-cols-2" @submit.prevent="saveIntake">
        <UAlert v-if="intakeForm.id && intakeForm.status !== 'DRAFT'" color="info" variant="soft" icon="i-lucide-lock-keyhole" title="Operational identity locked" description="Academic year and intake code cannot change after the intake leaves draft." class="sm:col-span-2" />
        <UFormField label="Academic year" required class="sm:col-span-2"><USelect v-model="intakeForm.academicYearId" :items="yearItems" value-key="value" class="w-full" :disabled="Boolean(intakeForm.id && intakeForm.status !== 'DRAFT')" /></UFormField>
        <UFormField label="Code" description="Use letters, numbers, hyphens, or underscores. Spaces are not allowed." required><UInput v-model="intakeForm.code" class="w-full" placeholder="JAN-2027" maxlength="50" :disabled="Boolean(intakeForm.id && intakeForm.status !== 'DRAFT')" /></UFormField>
        <UFormField label="Name" required><UInput v-model="intakeForm.name" class="w-full" placeholder="January 2027 Intake" /></UFormField>
        ...
        <UFormField label="Maximum programme choices" description="The most Programmes an applicant may rank for this intake." required class="sm:col-span-2"><UInput v-model.number="intakeForm.maximumProgrammeChoices" type="number" min="1" max="20" class="w-full" /></UFormField>
        <UFormField
          label="Programme Levels"
          description="Choose every applicant level available in this intake, such as Undergraduate or Postgraduate."
          required
          class="sm:col-span-2"
        >
          <USelectMenu
            v-model="intakeForm.programmeLevelIds"
            :items="programmeLevelItems"
            value-key="value"
            label-key="label"
            multiple
            aria-label="Programme Levels"
            placeholder="Search and select Programme Levels"
            :disabled="intakeEligibilityLocked"
            class="w-full"
          />
        </UFormField>
        <UFormField
          label="Specific Programmes"
          description="Optional. Leave empty to include every active Programme in the selected levels; select Programmes to narrow this intake."
          class="sm:col-span-2"
        >
          <USelectMenu
            v-model="intakeForm.programmeIds"
            :items="specificProgrammeItems"
            value-key="value"
            label-key="label"
            multiple
            aria-label="Specific Programmes"
            placeholder="All active Programmes in selected levels"
            :disabled="intakeEligibilityLocked || !intakeForm.programmeLevelIds.length"
            class="w-full"
          />
        </UFormField>
        <UAlert
          ...
          :description="intakeForm.programmeIds.length
            ? `Only the ${intakeForm.programmeIds.length} selected Programme${intakeForm.programmeIds.length === 1 ? '' : 's'} will be available for this intake.`
            : 'Every active Programme in the selected Programme Levels will be available.'"
          class="sm:col-span-2"
        />
        <UFormField v-if="intakeForm.id" label="Change reason" description="Required for the audited correction history." required class="sm:col-span-2"><UTextarea v-model="intakeForm.changeReason" class="w-full" :rows="3" minlength="10" maxlength="1000" placeholder="Explain the intake correction." /></UFormField>
      </form>
    </template>
    <template #footer><div class="flex w-full justify-end gap-2"><UButton label="Cancel" color="neutral" variant="outline" @click="intakeModalOpen = false" /><UButton type="submit" form="intake-form" :label="intakeForm.id ? 'Save changes' : 'Create intake'" icon="i-lucide-save" :loading="saving" /></div></template>
  </EmhareRecordDrawer>
</template>
