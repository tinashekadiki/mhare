...
const moduleDefinition = computed(() => operationalDashboardModules.find(module => module.key === moduleKey.value))
...
async function loadDashboard() {
  ...
  try {
    ...
    snapshot.value = null
    errorMessage.value = api.errorMessage(error, `${moduleDefinition.value?.label ?? 'This module'} overview could not be loaded.`)
  } finally {
    loading.value = false
  }
}
  ...
  <UDashboardPanel>
    ...
    <template #body>
      ...
      >
        ...
        >
          <template #actions>
            <UButton label="Try again" icon="i-lucide-refresh-cw" color="error" variant="soft" @click="loadDashboard" />
          </template>
        </UAlert>

        <div v-if="loading && !snapshot" aria-label="Loading operational dashboard" class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
