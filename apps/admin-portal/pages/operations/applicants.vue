...
const statusItems = [
  ...
  ...[
    'DRAFT', 'SUBMITTED', 'PAYMENT_PENDING', 'UNDER_REVIEW', 'INCOMPLETE', 'ELIGIBLE', 'NOT_ELIGIBLE',
    'SHORTLISTED', 'SELECTED', 'OFFERED', 'ACCEPTED', 'DECLINED', 'WITHDRAWN', 'CONVERTED'
...
const drawerDescription = computed(() => drawerMode.value === 'view'
...
async function loadApplicantDetails(applicantId: string) {
  ...
  detailsError.value = ''
  try {
    applicantDetails.value = await api.request<ApplicantDetails>(`/api/admissions/applicants/${applicantId}`)
  } catch (error) {
...
async function saveProfile() {
  ...
  try {
    applicantDetails.value = await api.request<ApplicantDetails>(`/api/admissions/applicants/${profile.id}`, {
      method: 'PUT',
      body: {
    ...
    drawerMode.value = 'view'
    await showSuccess('Profile updated', `${applicantDetails.value.profile.applicantNumber} was corrected.`)
...
function applicationStatusTone(status: string) {
  ...
  if (status === 'OFFERED' || status === 'ACCEPTED' || status === 'ELIGIBLE') return 'success' as const
  if (status === 'DECLINED' || status === 'WITHDRAWN' || status === 'NOT_ELIGIBLE') return 'error' as const
...
<template>
  <UDashboardPanel>
    <template #header>
      ...
      <UDashboardToolbar>
        <template #left>
          <UInput v-model="search" icon="i-lucide-search" placeholder="Search name, applicant number, or email" class="w-full sm:w-96" />
        </template>
        <template #right>
      ...
      <div class="space-y-4 p-4 sm:p-6">
          ...
          >
            <template #applicantNumber-cell="{ row }">
              <button type="button" class="text-left" @click="openApplicant(row.original)">
                <p class="font-medium text-highlighted underline-offset-2 hover:underline">{{ row.original.displayName }}</p>
                <p class="mt-1 font-mono text-xs text-muted">{{ row.original.applicantNumber }}</p>
              </button>
            </template>

            <template #applicantCategoryCode-cell="{ row }">
              <EmhareStatusPill :label="categoryLabel(row.original.applicantCategoryCode)" tone="neutral" />
            </template>

            <template #primaryEmail-cell="{ row }">
              <div>
                <p class="text-sm text-highlighted">{{ row.original.primaryEmail }}</p>
                <p v-if="row.original.primaryPhone" class="mt-1 text-xs text-muted">{{ row.original.primaryPhone }}</p>
              </div>
            </template>
            ...
            <template #latestApplicationNumber-cell="{ row }">
              <div v-if="row.original.latestApplicationNumber">
                ...
                <p class="mt-1 text-xs text-muted">{{ formatStatus(row.original.latestApplicationStatus ?? '') }} · {{ row.original.latestIntakeCode }}</p>
              </div>
            ...
            <template #actions-cell="{ row }">
              <div class="flex justify-end">
                <UButton label="View" icon="i-lucide-eye" color="neutral" variant="ghost" @click="openApplicant(row.original)" />
              </div>
            </template>
            ...
            <template #empty>
              <div class="py-10">
                ...
                <p class="mt-1 text-sm text-muted">Adjust the search, category, or application status filter.</p>
              </div>
            </template>
          </EmharePaginatedTable>
        </div>
        ...
        >
          <template #body>
            ...
            <UAlert v-else-if="detailsError" color="error" variant="soft" title="Profile unavailable" :description="detailsError" />

            <div v-else-if="drawerMode === 'view' && applicantDetails" class="space-y-5">
              <div class="grid gap-3 sm:grid-cols-3">
                <EmhareKpiCard label="Profile completeness" :value="`${applicantDetails.profile.completenessPercentage}%`" icon="i-lucide-user-round-check" :tone="completenessTone(applicantDetails.profile.completenessPercentage)" />
                <EmhareKpiCard label="Applications" :value="applicantDetails.applications.length" icon="i-lucide-files" tone="primary" />
                <EmhareKpiCard label="Category" :value="categoryLabel(applicantDetails.profile.applicantCategoryCode)" icon="i-lucide-tag" tone="neutral" />
              </div>
              ...
              <div class="grid gap-3 sm:grid-cols-2">
                <div><p class="text-xs text-muted">Name</p><p class="text-sm text-highlighted">{{ applicantDetails.profile.firstName }} {{ applicantDetails.profile.middleNames }} {{ applicantDetails.profile.lastName }}</p></div>
                <div><p class="text-xs text-muted">Applicant number</p><p class="font-mono text-sm text-highlighted">{{ applicantDetails.profile.applicantNumber }}</p></div>
                <div><p class="text-xs text-muted">Primary email</p><p class="text-sm text-highlighted">{{ applicantDetails.profile.primaryEmail }}</p></div>
                <div><p class="text-xs text-muted">Primary phone</p><p class="text-sm text-highlighted">{{ applicantDetails.profile.primaryPhone ?? 'Not set' }}</p></div>
                <div><p class="text-xs text-muted">Date of birth</p><p class="text-sm text-highlighted">{{ applicantDetails.profile.dateOfBirth ?? 'Not set' }}</p></div>
                <div><p class="text-xs text-muted">Gender</p><p class="text-sm text-highlighted">{{ applicantDetails.profile.genderCode ?? 'Not set' }}</p></div>
                <div><p class="text-xs text-muted">National ID</p><p class="text-sm text-highlighted">{{ applicantDetails.profile.nationalIdNumber ?? 'Not set' }}</p></div>
                <div><p class="text-xs text-muted">Passport number</p><p class="text-sm text-highlighted">{{ applicantDetails.profile.passportNumber ?? 'Not set' }}</p></div>
                <div><p class="text-xs text-muted">Residential address</p><p class="text-sm text-highlighted">{{ applicantDetails.profile.residentialAddress ?? 'Not set' }}</p></div>
                <div><p class="text-xs text-muted">Postal address</p><p class="text-sm text-highlighted">{{ applicantDetails.profile.postalAddress ?? 'Not set' }}</p></div>
              </div>

              <div>
                <p class="mb-2 text-sm font-medium text-highlighted">Applications</p>
                <div v-if="!applicantDetails.applications.length" class="rounded-md border border-muted p-4 text-center text-sm text-muted">
                  No applications submitted yet.
                </div>
                <div v-else class="space-y-2">
                  <div v-for="application in applicantDetails.applications" :key="application.id" class="flex items-center justify-between rounded-md border border-muted p-3">
                    <div>
                      <p class="text-sm font-medium text-highlighted">{{ application.applicationNumber }}</p>
                      <p class="mt-1 text-xs text-muted">{{ application.intakeCode }} · {{ application.applicationTypeName }}</p>
                    </div>
            ...
            <div v-else-if="drawerMode === 'edit'" class="grid gap-4 sm:grid-cols-2">
              <EmhareFormField v-model="profileForm.applicantCategoryCode" type="select" name="applicantCategoryCode" label="Category" :items="profileCategoryItems" required />
              <EmhareFormField v-model="profileForm.titleCode" name="titleCode" label="Title" />
              <EmhareFormField v-model="profileForm.firstName" name="firstName" label="First name" required />
              <EmhareFormField v-model="profileForm.middleNames" name="middleNames" label="Middle names" />
              <EmhareFormField v-model="profileForm.lastName" name="lastName" label="Last name" required />
              ...
              <EmhareFormField v-model="profileForm.maritalStatusCode" name="maritalStatusCode" label="Marital status" />
              <EmhareFormField v-model="profileForm.nationalIdNumber" name="nationalIdNumber" label="National ID number" />
              <EmhareFormField v-model="profileForm.passportNumber" name="passportNumber" label="Passport number" />
              <EmhareFormField v-model="profileForm.placeOfBirth" name="placeOfBirth" label="Place of birth" />
              <EmhareFormField v-model="profileForm.disabilityStatusCode" name="disabilityStatusCode" label="Disability status" />
              <EmhareFormField v-model="profileForm.sponsorTypeCode" name="sponsorTypeCode" label="Sponsor type" />
              <EmhareFormField v-model="profileForm.primaryEmail" type="email" name="primaryEmail" label="Primary email" required />
              <EmhareFormField v-model="profileForm.primaryPhone" type="phone" name="primaryPhone" label="Primary phone" />
              <EmhareFormField v-model="profileForm.postalAddress" type="textarea" name="postalAddress" label="Postal address" class="sm:col-span-2" />
              <EmhareFormField v-model="profileForm.residentialAddress" type="textarea" name="residentialAddress" label="Residential address" class="sm:col-span-2" />
              <EmhareFormField v-model="profileForm.specialNeeds" type="textarea" name="specialNeeds" label="Special needs" class="sm:col-span-2" />
              <EmhareFormField
          ...
          <template #footer>
            <div class="flex w-full items-center justify-between gap-3">
              <EmhareGuidedActionButton
                ...
                :guidance-instructions="applicantDetails ? [] : ['Wait for the applicant profile to finish loading before editing it.']"
                @click="beginEdit"
              />
              <UButton v-else label="Back to profile" icon="i-lucide-arrow-left" color="neutral" variant="ghost" @click="cancelEdit" />
              <div class="flex gap-3">
                <UButton label="Close" color="neutral" variant="outline" @click="drawerOpen = false" />
                <EmhareGuidedActionButton
                  ...
                  :guidance-instructions="[...(!profileForm.firstName.trim() ? ['Enter the first name.'] : []), ...(!profileForm.lastName.trim() ? ['Enter the last name.'] : []), ...(!profileForm.primaryEmail.trim() ? ['Enter the primary email address.'] : []), ...(profileForm.changeReason.trim().length < 10 ? ['Provide at least 10 characters of change evidence.'] : [])]"
                  @click="saveProfile"
                />
              </div>
            </div>
