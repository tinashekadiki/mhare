...
import type {
  ...
  CurriculumModuleUsageSummary,
  ProgrammeVersionSummary,
} from "@emhare/portal-shell/types/academic";
...
const selectedProgrammeId = ref(
  typeof route.query.programmeId === "string" ? route.query.programmeId : "",
);
...
const selectedProgramme = computed(() =>
  programmes.value.find((programme) => programme.id === selectedProgrammeId.value),
);
const yearOfStudyOptions = computed(() =>
  yearOfStudyItems(selectedProgramme.value?.maximumDurationPeriods ?? 16),
);
const selectedVersion = computed(() =>
  programmeVersions.value.find((version) => version.id === selectedVersionId.value),
);
const availableModuleItems = computed(() => {
  ...
  return modules.value
    ...
    .map((module) => ({
      label: `${module.code} · ${module.name} · ${Number(module.creditValue).toFixed(2)} credits`,
      value: module.id,
    }));
});
const newVersionGuidance = computed(() => {
  ...
  if (!selectedProgrammeId.value)
    return ["Select the programme whose curriculum you want to version."];
  return [];
});
const addModuleGuidance = computed(() => {
  ...
  return [
    "Every active Module is already attached to this curriculum version. Create or activate another Module, or select a different version.",
  ];
});
const approveVersionGuidance = computed(() =>
  curriculumModules.value.length
    ? []
    : ["Add at least one Module to this draft curriculum before approving it."],
);
const totalCredits = computed(() =>
  curriculumModules.value.reduce((total, item) => total + Number(item.creditValue), 0),
);
const versionStatusItems = computed(() =>
  programmeVersions.value.map((version) => ({
...
onMounted(async () => {
  try {
    ...
    if (!selectedProgrammeId.value && programmes.value.length)
      selectedProgrammeId.value = programmes.value[0]!.id;
  } catch {
    // Render shared error.
  }
});
...
watch(
  versionModalOpen,
  (open) =>
    open && Object.assign(versionForm, { versionCode: "", effectiveFrom: "", effectiveTo: "" }),
);
watch(curriculumModalOpen, (open) => {
  if (open && curriculumDrawerMode.value === "add")
    Object.assign(curriculumForm, {
      moduleId: availableModuleItems.value[0]?.value ?? "",
      yearOfStudy: 1,
      semesterNumber: 1,
      moduleType: "COMPULSORY",
      creditValue: 12,
      minimumMarkRequired: 50,
      sortOrder: curriculumModules.value.length + 1,
      changeReason: "",
    });
});
watch(
  () => curriculumForm.moduleId,
  (moduleId) => {
...
async function loadProgrammeVersions(programmeId: string) {
  ...
  try {
    programmeVersions.value = await api.request<ProgrammeVersionSummary[]>(
      `/api/academic/programmes/${programmeId}/versions`,
    );
...
async function loadCurriculum(versionId: string) {
  loadingDetail.value = true;
  try {
    curriculumModules.value = await api.request<CurriculumModuleSummary[]>(
      `/api/academic/programme-versions/${versionId}/curriculum`,
    );
  } catch (error) {
...
async function createVersion() {
  ...
  saving.value = true;
  try {
    const created = await api.request<ProgrammeVersionSummary>(
      `/api/academic/programmes/${selectedProgrammeId.value}/versions`,
      {
        method: "POST",
        body: { ...versionForm, effectiveTo: versionForm.effectiveTo || null },
      },
    );
    ...
    toast.add({
      title: "Draft curriculum version created",
      color: "success",
  ...
  try {
    await api.request(`/api/academic/programme-versions/${selectedVersionId.value}/curriculum`, {
      ...
      body: {
        ...
        periodNumber: toProgrammePeriodNumber(
          curriculumForm.yearOfStudy,
          curriculumForm.semesterNumber,
        ),
    ...
    toast.add({
      title: "Module added to curriculum",
      color: "success",
...
function openCurriculumModuleAmendment(curriculumModule: CurriculumModuleSummary) {
  ...
  Object.assign(curriculumForm, {
    ...
    minimumMarkRequired:
      curriculumModule.minimumMarkRequired == null
        ? 50
        : Number(curriculumModule.minimumMarkRequired),
...
async function updateCurriculumModule() {
  ...
  try {
    await api.request(
      `/api/academic/programme-versions/${selectedVersionId.value}/curriculum/${curriculumModule.id}`,
      {
        method: "PUT",
        body: {
          periodNumber: toProgrammePeriodNumber(
    ...
    curriculumModalOpen.value = false;
    toast.add({
      title: "Curriculum Module amended",
      description: curriculumModule.moduleCode,
      color: "success",
...
async function removeCurriculumModule(curriculumModule: CurriculumModuleSummary) {
  ...
  try {
    ...
    const result = await Swal.fire({
      ...
      inputValidator: (value) =>
        value.trim().length >= 10
          ? undefined
          : "Provide at least 10 characters of amendment evidence.",
      icon: "warning",
    ...
    await api.request(
      `/api/academic/programme-versions/${selectedVersionId.value}/curriculum/${curriculumModule.id}/removal`,
      {
        method: "POST",
    ...
    await refreshSelectedVersion();
    toast.add({
      title: "Curriculum Module removed",
      description: curriculumModule.moduleCode,
      color: "success",
...
async function approveVersion(version: ProgrammeVersionSummary) {
  ...
  try {
    await api.request(`/api/academic/programme-versions/${version.id}/approve`, {
      method: "POST",
      body: { expectedVersion: version.version },
    });
    await refreshSelectedVersion();
    toast.add({
      title: "Curriculum version approved",
      description: "The version is operational and remains governed by amendment safeguards.",
      color: "success",
...
async function retireVersion(version: ProgrammeVersionSummary) {
  ...
  try {
    await api.request(`/api/academic/programme-versions/${version.id}/retire`, {
      method: "POST",
      body: { expectedVersion: version.version, retirementDate: result.value },
    });
    await refreshSelectedVersion();
...
<template>
  <UDashboardPanel>
      ...
      <UDashboardNavbar title="Curriculum versions">
        ...
        <template #right
          ><EmhareGuidedActionButton
            aria-label="New curriculum version"
            label="New version"
            icon="i-lucide-git-branch-plus"
            color="primary"
            class="[&_[data-slot=label]]:hidden sm:[&_[data-slot=label]]:inline"
            guidance-title="Curriculum version setup required"
            :guidance-instructions="newVersionGuidance"
            :guidance-action-label="!programmes.length ? 'Open Programmes' : undefined"
            @guidance-action="navigateTo('/operations/programmes')"
      ...
      <UDashboardToolbar>
        <template #left
          ><USelectMenu
            ...
            :search-input="{ placeholder: 'Search programmes' }"
            class="w-full sm:w-[30rem]"
        /></template>
        <template #right
          ><USelect
            v-model="selectedVersionId"
            :items="versionStatusItems"
            value-key="value"
            placeholder="Select version"
            class="w-72"
            :disabled="!programmeVersions.length"
        /></template>
      </UDashboardToolbar>
    </template>
    ...
    <template #body>
      <div class="space-y-5 p-4 sm:p-6">
        <UAlert
          v-if="academicSetup.loadError.value"
          color="error"
          variant="soft"
          icon="i-lucide-circle-alert"
          title="Curriculum setup unavailable"
          :description="academicSetup.loadError.value"
        />
        <UAlert
          v-if="!programmes.length && !academicSetup.loading.value"
          color="warning"
          variant="soft"
          icon="i-lucide-graduation-cap"
          title="No programme selected"
          description="Create a programme before building its curriculum."
        />
        ...
        <template v-if="selectedProgramme">
          <div class="grid gap-3 sm:grid-cols-4">
            <EmhareKpiCard
              ...
              :hint="selectedProgramme.name"
              icon="i-lucide-graduation-cap"
              tone="primary"
            />
            <EmhareKpiCard
              label="Versions"
              :value="programmeVersions.length"
              icon="i-lucide-git-branch"
              tone="primary"
            />
            <EmhareKpiCard
              label="Curriculum Modules"
              :value="curriculumModules.length"
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div class="flex items-center gap-2">
                  <h2 class="font-semibold text-highlighted">
                    Version {{ selectedVersion.versionCode }}
                  </h2>
                  <EmhareStatusPill
                    :label="selectedVersion.status"
                    :tone="versionTone(selectedVersion.status)"
                  />
                </div>
                <p class="mt-1 text-sm text-muted">
                  Effective {{ selectedVersion.effectiveFrom
                  }}<span v-if="selectedVersion.effectiveTo">
                    to {{ selectedVersion.effectiveTo }}</span
                  ><span v-else> with no scheduled end</span>
                </p>
                <p v-if="selectedVersion.approvedAt" class="mt-1 text-xs text-muted">
                  Approved {{ new Date(selectedVersion.approvedAt).toLocaleString("en-ZW") }}
                </p>
              </div>
              <div class="flex flex-wrap gap-2">
                <EmhareGuidedActionButton
                  ...
                  color="neutral"
                  variant="outline"
                  guidance-title="Module setup required"
                  :guidance-instructions="addModuleGuidance"
                  guidance-action-label="Open Modules"
                  @guidance-action="navigateTo('/operations/modules')"
                ...
                <EmhareGuidedActionButton
                  ...
                  icon="i-lucide-shield-check"
                  color="primary"
                  guidance-title="Curriculum cannot be approved yet"
                  :guidance-instructions="approveVersionGuidance"
                  :loading="activeVersionActionId === selectedVersion.id"
                  @click="approveVersion(selectedVersion)"
                />
                <UButton
                  v-if="selectedVersion.status === 'APPROVED'"
                  label="Retire version"
                  icon="i-lucide-archive"
                  color="warning"
                  variant="outline"
                  :loading="activeVersionActionId === selectedVersion.id"
                  @click="retireVersion(selectedVersion)"
                />
              </div>
            </div>
          </UCard>
          ...
          <UAlert
            ...
            variant="soft"
            icon="i-lucide-shield-check"
            title="Approved curriculum remains amendable"
            description="You may add Modules or amend placement details. Removal is permitted only when Student Records and Results both confirm that the Module has no usage."
          />
          <UAlert
            v-if="selectedVersion?.status === 'DRAFT' && addModuleGuidance.length"
            color="warning"
            variant="soft"
            icon="i-lucide-list-plus"
            title="No Module is currently available to add"
            :description="addModuleGuidance[0]"
            :actions="[
              {
                label: 'Open Modules',
          ...
          <UCard :ui="{ body: 'p-0' }">
            <template #header
              ><div>
                <h2 class="font-semibold text-highlighted">Curriculum Module placement</h2>
                <p class="text-sm text-muted">
                  Year, semester, requirement, credits, ordering, and minimum pass rules.
                </p>
              </div></template
            >
            <EmharePaginatedTable
              ...
              sticky
            >
              <template #moduleCode-cell="{ row }"
                ><span class="font-mono font-semibold text-primary">{{
                  row.original.moduleCode
                }}</span></template
              >
              <template #periodNumber-cell="{ row }"
                ><span class="whitespace-nowrap">{{
                  studyPeriodLabel(row.original.periodNumber)
                }}</span></template
              >
              <template #moduleType-cell="{ row }"
                ><EmhareStatusPill
                  :label="row.original.moduleType"
                  :tone="row.original.moduleType === 'COMPULSORY' ? 'info' : 'neutral'"
              /></template>
              <template #creditValue-cell="{ row }">{{
                Number(row.original.creditValue).toFixed(2)
              }}</template>
              <template #minimumMarkRequired-cell="{ row }">{{
                row.original.minimumMarkRequired == null
                  ? "Institution rule"
                  : `${Number(row.original.minimumMarkRequired).toFixed(2)}%`
              }}</template>
              <template #actions-cell="{ row }">
                <div v-if="selectedVersion?.status !== 'RETIRED'" class="flex justify-end gap-1">
                  <UButton
                    ...
                    color="neutral"
                    variant="ghost"
                    @click="openCurriculumModuleAmendment(row.original)"
                  />
                  <UButton
                    label="Remove"
                    icon="i-lucide-list-minus"
                    color="error"
                    variant="ghost"
                    @click="removeCurriculumModule(row.original)"
                  />
                </div>
              ...
              <template #empty
                ><div class="py-12 text-center">
                  <UIcon name="i-lucide-list-x" class="mx-auto size-8 text-muted" />
                  <p class="mt-3 font-medium text-highlighted">No Modules in this version</p>
                  <p class="mt-1 text-sm text-muted">
                    {{
                      addModuleGuidance[0] ??
                      "Add an active Module to this governed curriculum version."
                    }}
                  </p>
                </div></template
              >
  ...
  <EmhareRecordDrawer
    ...
    title="Create programme version"
    description="Start a new governed curriculum lifecycle in draft."
  >
    <template #body
      ><form id="version-form" class="grid gap-4 sm:grid-cols-2" @submit.prevent="createVersion">
        <UFormField label="Version code" required class="sm:col-span-2"
          ><UInput
            v-model="versionForm.versionCode"
            class="w-full"
            placeholder="2027.1" /></UFormField
        ><UFormField label="Effective from" required
          ><UInput v-model="versionForm.effectiveFrom" type="date" class="w-full" /></UFormField
        ><UFormField label="Effective to"
          ><UInput v-model="versionForm.effectiveTo" type="date" class="w-full"
        /></UFormField></form
    ></template>
    <template #footer
      ><UButton
        ...
        color="neutral"
        variant="outline"
        @click="versionModalOpen = false" /><UButton
        type="submit"
        form="version-form"
  ...
  <EmhareRecordDrawer
    v-model:open="curriculumModalOpen"
    presentation="page"
    :title="curriculumDrawerTitle"
    :description="curriculumDrawerDescription"
  >
    <template #body>
      <form
        id="curriculum-form"
        class="grid gap-4 sm:grid-cols-2"
        @submit.prevent="saveCurriculumModule"
      >
        <UFormField
          v-if="curriculumDrawerMode === 'add'"
          label="Module"
          required
          class="sm:col-span-2"
          ><USelect
            v-model="curriculumForm.moduleId"
        ...
        <UAlert
          ...
          :title="`${selectedCurriculumModule?.moduleCode} · ${selectedCurriculumModule?.moduleName}`"
          description="The Module identity remains fixed; amend its placement and academic rules below."
          class="sm:col-span-2"
        />
        <UFormField label="Year of study" required
          ><USelect
            v-model="curriculumForm.yearOfStudy"
        ...
        <UFormField label="Semester" required
          ><USelect
            v-model="curriculumForm.semesterNumber"
            :items="semesterItems"
            value-key="value"
            class="w-full"
        /></UFormField>
        <UFormField label="Requirement type" required
          ><USelect
            v-model="curriculumForm.moduleType"
            :items="[
            ...
            class="w-full"
        /></UFormField>
        <UFormField label="Credit value" required
          ><UInput
            v-model.number="curriculumForm.creditValue"
            type="number"
            min="0.01"
            step="0.01"
            class="w-full"
        ...
        <UFormField label="Minimum pass mark"
          ><UInput
            v-model.number="curriculumForm.minimumMarkRequired"
            type="number"
            min="0"
            max="100"
            step="0.01"
        ...
        <UFormField label="Sort order" required
          ><UInput v-model.number="curriculumForm.sortOrder" type="number" min="1" class="w-full"
        /></UFormField>
        <UFormField
          label="Amendment reason"
          description="Stored with the audited curriculum revision."
          required
          class="sm:col-span-2"
          ><UTextarea
            v-model="curriculumForm.changeReason"
            :rows="3"
            autoresize
            :maxrows="6"
            class="w-full"
            placeholder="Record the curriculum committee decision, correction evidence, or implementation reason."
        /></UFormField>
      </form>
    </template>
    <template #footer
      ><UButton
        ...
        color="neutral"
        variant="outline"
        @click="curriculumModalOpen = false" /><UButton
        type="submit"
        form="curriculum-form"
        :label="curriculumDrawerMode === 'add' ? 'Add Module' : 'Save amendment'"
        :icon="curriculumDrawerMode === 'add' ? 'i-lucide-list-plus' : 'i-lucide-save'"
        :loading="saving"
        :disabled="curriculumForm.changeReason.trim().length < 10"
    /></template>
  </EmhareRecordDrawer>
</template>
