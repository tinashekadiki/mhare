...
vi.mock('@emhare/portal-shell/utils/operational-dashboard', async importOriginal => {
  const actual = await importOriginal<typeof import('../../../../packages/portal-shell/utils/operational-dashboard')>()
  return {
    ...actual,
...
const financeSnapshot: OperationalDashboardSnapshot = {
  ...
  metrics: [
    { label: 'Posted invoices', value: 4, hint: 'Governed invoice records', icon: 'i-lucide-receipt', tone: 'success' },
    { label: 'Student accounts', value: 3, hint: 'Finance-owned accounts', icon: 'i-lucide-wallet-cards', tone: 'primary' }
  ],
  actions: [
    { label: 'Unrated payments', value: 2, description: 'Payments awaiting an effective exchange rate.', icon: 'i-lucide-scale', to: '/operations/finance-collections' }
  ],
  distribution: [
  ...
  links: [
    { label: 'Cash collections', description: 'Reconcile receipts and payments.', icon: 'i-lucide-banknote', to: '/operations/finance-collections' }
  ]
}
...
const KpiStub = defineComponent({
  ...
  template: '<article class="kpi-stub"><h3>{{ label }}</h3><strong>{{ value }}</strong><p>{{ hint }}</p><span>{{ tone }}</span></article>'
})
...
describe('Main Operations dashboard page', () => {
  ...
  beforeEach(() => {
    ...
    dashboardLoaderMocks.loadOperationsOverview.mockImplementation(async (_api: unknown, keys: string[]) => [{
      ...financeSnapshot,
      key: keys[0],
      label: keys[0] === 'finance' ? 'Finance' : `Live ${keys[0]}`,
      dashboardPath: keys[0] === 'admissions'
  ...
  it('renders all module destinations progressively and identifies unavailable services explicitly', async () => {
    ...
    expect(wrapper.get('[data-testid="operations-dashboard-content"]').classes()).toContain('max-w-none')
    expect(wrapper.text()).toContain('Notifications cannot be read.')
    expect(wrapper.text()).toContain('Unavailable')
    expect(wrapper.text()).toContain('Updated')
    expect(wrapper.findAll('a').some(link => link.attributes('href') === '/operations/admissions-dashboard')).toBe(true)
  })
...
describe('Operational module dashboard page', () => {
  ...
  it('loads a governed module snapshot and exposes its full-width workspace', async () => {
    ...
    expect(dashboardLoaderMocks.loadOperationalDashboard).toHaveBeenCalledWith(expect.anything(), 'finance')
    expect(wrapper.text()).toContain('Finance overview')
    expect(wrapper.text()).toContain('Finance dashboard')
    expect(wrapper.text()).toContain('Updated')
    expect(wrapper.get('[data-testid="operational-dashboard-finance"]').classes()).toContain('max-w-none')
  ...
  it('reloads for a valid route change and ignores an invalid watched key', async () => {
    ...
    await nextTick()
    await flushPromises()
    expect(dashboardLoaderMocks.loadOperationalDashboard).toHaveBeenLastCalledWith(expect.anything(), 'documents')
  ...
  it('rejects an unknown dashboard key with a 404', async () => {
    ...
    expect(() => mount(DashboardPage, { global: { stubs: commonStubs } })).toThrow('Operational dashboard not found')
  })
})
...
describe('Shared operational dashboard presentation', () => {
  it('renders metrics, queues, governed state bars and workspace destinations', async () => {
    const Dashboard = (await import('../../../../packages/portal-shell/components/domain/operations/EmhareOperationalDashboard.vue')).default
    ...
    expect(wrapper.findAll('[style]').map(element => element.attributes('style'))).toEqual(expect.arrayContaining([
