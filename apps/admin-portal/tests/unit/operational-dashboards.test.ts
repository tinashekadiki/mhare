...
describe('Operational dashboards', () => {
  it('catalogues every implemented operational domain with a dedicated dashboard destination', () => {
    ...
    expect(new Set(operationalDashboardModules.map(module => module.dashboardPath)).size).toBe(11)
    expect(operationalDashboardModules.find(module => module.key === 'admissions')?.dashboardPath)
  ...
  it('derives Finance workload from governed pricing, billing and collection state', async () => {
    ...
    expect(snapshot.actions.map(action => [action.label, action.value])).toEqual(expect.arrayContaining([
      ['Pricing attention', 1],
      ['Billing approval', 1],
  ...
  it('keeps the main dashboard available when one service cannot provide metrics', async () => {
    const request = vi.fn(async (path: string) => {
      ...
      if (path === '/api/academic/overview') {
        return {
          academicUnitTypes: [], academicUnits: [], academicYears: [], academicPeriodTypes: [],
          academicPeriods: [], intakes: [], programmeLevels: [], programmeTypes: [], programmes: [], modules: []
        }
      }
  ...
  it('builds live snapshots for every implemented service contract', async () => {
    const responses: Record<string, unknown> = {
      ...
      '/api/core/workflows/tasks': ['OPEN', 'CLAIMED', 'COMPLETED', 'CANCELLED'].map(status => ({ status })),
      ...
      '/api/admissions/reports/pipeline-summary': {
        ...
        paymentCounts: [{ code: 'PAID', count: 1 }, { code: 'WAIVED', count: 1 }, { code: 'PENDING', count: 2 }],
        categoryCounts: [], genderCounts: [],
        rankedChoiceCounts: [{ rank: 1, choices: 3 }],
        intakeStatistics: [], programmeStatistics: [],
        filterOptions: { intakes: [], applicationTypes: [], programmes: [], categories: [], genders: [] }
      },
      ...
      '/api/finance/collections': {
        ...
        receipts: [], allocations: [], creditNotes: [{ status: 'DRAFT' }, { status: 'POSTED' }]
      },
      '/api/finance/collections/accounts': [{ id: 'account-1' }],
      '/api/student-records/conversions': [
        { status: 'COMPLETED' }, { status: 'PROVISIONING' }, { status: 'FAILED' }
      ],
      '/api/student-records/registrations': [
        { status: 'DRAFT', modules: [] },
        { status: 'SUBMITTED', modules: [] },
        { status: 'ACADEMIC_APPROVED', modules: [] },
        { status: 'CONFIRMED', modules: [{ id: 'module-1' }, { id: 'module-2' }] }
      ],
      ...
      '/api/results/progression/decisions': [
        { status: 'CALCULATED' }, { status: 'REVIEWED' }, { status: 'APPROVED' }, { status: 'PUBLISHED' }, { status: 'REJECTED' }
      ],
      '/api/exams/setup': {
        venueTypes: [],
      ...
      '/api/timetabling/runs': ['GENERATED', 'REVIEWED', 'APPROVED', 'PUBLISHED', 'REJECTED'].map(status => ({ status })),
      '/api/exams/invigilation': {
        venueOperations: [
          { attendanceSession: { status: 'OPEN', incidents: [{ status: 'REPORTED' }, { status: 'RESOLVED' }] } },
          { attendanceSession: { status: 'CLOSED', incidents: [] } },
          { attendanceSession: null }
        ]
      },
      '/api/accommodation/setup': {
        premises: [], roomTypes: [], residenceHalls: [], applicationPeriods: [],
        rooms: [
      ...
      '/api/accommodation/operations': {
        rates: [{ status: 'DRAFT', ratingStatus: 'UNRATED' }, { status: 'ACTIVE', ratingStatus: 'RATED' }],
      ...
      '/api/dining/setup': {
        diningHalls: [{ active: true }, { active: false }], mealOptions: [], serviceTimes: [],
        diningPlans: [{ status: 'DRAFT' }, { status: 'ACTIVE' }], planMeals: [], hallAssignmentRules: [], attendantAssignments: []
      },
      '/api/dining/operations': {
        assignments: [{ status: 'ACTIVE' }, { status: 'ENDED' }],
        dietaryRequirements: [
          { status: 'ACTIVE', severity: 'CRITICAL' }, { status: 'ACTIVE', severity: 'INFORMATION' }, { status: 'RESOLVED', severity: 'CRITICAL' }
        ],
        sessions: [{ status: 'PLANNED' }, { status: 'OPEN' }, { status: 'CLOSED' }],
        attendanceEvents: [
          { outcome: 'ADMITTED', reversed: false }, { outcome: 'ADMITTED', reversed: true }, { outcome: 'DENIED', reversed: false }
        ],
        reversals: [], workflowEvents: [], attendanceStatistics: []
      },
      ...
      '/api/notifications': {
        templates: [{ status: 'ACTIVE' }, { status: 'DRAFT' }], consents: [], deliveryAttempts: [],
        requests: [
          { status: 'QUEUED' }, { status: 'PROCESSING' }, { status: 'RETRY_SCHEDULED' }, { status: 'SENT' }, { status: 'FAILED' }
        ],
    ...
    expect(overview.find(module => module.key === 'exams-timetabling')?.actions)
      .toEqual(expect.arrayContaining([expect.objectContaining({ label: 'Unresolved incidents', value: 1 })]))
    expect(overview.find(module => module.key === 'documents')?.actions)
      .toEqual(expect.arrayContaining([expect.objectContaining({ label: 'Retry official records', value: 1 })]))
    expect(overview.find(module => module.key === 'notifications')?.actions)
      .toEqual(expect.arrayContaining([expect.objectContaining({ label: 'Dead integration events', value: 1 })]))
  })
})
