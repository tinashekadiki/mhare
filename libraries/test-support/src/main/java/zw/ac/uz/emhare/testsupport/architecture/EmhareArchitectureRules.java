...
public final class EmhareArchitectureRules {
  ...
  public static ArchRule auditableEntitiesMustEnforceSoftDelete() {
    return classes()
        .that()
        .areAnnotatedWith("jakarta.persistence.Entity")
        .and()
        .areAssignableTo(AuditableEntity.class)
        .should(
            new ArchCondition<>("enforce the canonical deleted_at visibility restriction") {
              @Override
              public void check(JavaClass entityClass, ConditionEvents events) {
                if (!entityClass.isAnnotatedWith(SQLRestriction.class)) {
                  events.add(
                      SimpleConditionEvent.violated(
                          entityClass,
                          entityClass.getFullName()
                              + " must declare @SQLRestriction(\"deleted_at IS NULL\")"));
                  return;
                }
                String restriction = entityClass.getAnnotationOfType(SQLRestriction.class).value();
                if (!"deleted_at IS NULL".equals(restriction)) {
                  events.add(
                      SimpleConditionEvent.violated(
                          entityClass,
                          entityClass.getFullName()
                              + " uses a non-standard soft-delete restriction: "
                              + restriction));
                }
              }
            })
  ...
  public static ArchRule technicalEntitiesMustUseInfrastructureModelPackages() {
    return classes()
        ...
        .should()
        .resideInAPackage("..infrastructure..model..")
        .allowEmptyShould(true);
  }
  ...
  public static ArchRule controllersMustNotAcceptApplicationCommands() {
    return methods()
        ...
        .areAnnotatedWith(RestController.class)
        .should(
            new ArchCondition<>("not accept application commands as HTTP parameters") {
              @Override
  ...
  public static ArchRule serviceMustNotImportAnotherBusinessService(String serviceBasePackage) {
    List<String> businessServicePackages =
        new ArrayList<>(
            List.of(
    ...
    return noClasses()
        .that()
        .resideInAPackage(serviceBasePackage + "..")
        .should()
        .dependOnClassesThat()
        .resideInAnyPackage(businessServicePackages.toArray(String[]::new));
  }
}
