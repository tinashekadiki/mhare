...
public class IntegrationMessagingMonitor implements HealthIndicator, MeterBinder {
  ...
  public void bindTo(MeterRegistry registry) {
    Gauge.builder(
            "emhare.integration.outbox.pending", this, monitor -> monitor.safeCount("PENDING"))
  ...
  private long countOutboxEvents(String status) {
    Long count =
        jdbcTemplate.queryForObject(
            "SELECT count(*) FROM integration_outbox WHERE status = ?", Long.class, status);
    return count == null ? 0 : count;
  }

  private Instant oldestPendingAt() {
    Timestamp oldestPendingTimestamp =
        jdbcTemplate.queryForObject(
