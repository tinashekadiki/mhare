      ...
      >
        <button
          ...
          :class="
            step.disabled
              ...
              : step.id === currentStep
                ? 'border-uzazure-700 text-uzazure-950'
                : 'border-transparent text-muted hover:border-uzazure-200 hover:text-uzazure-900'
          "
        ...
        >
            ...
            :class="
              step.status === 'complete'
                ...
                : step.status === 'attention'
                  ? 'bg-uzorange-100 text-uzorange-900'
                  : step.id === currentStep
                    ? 'bg-uzazure-100 text-uzazure-800'
                    : 'bg-muted text-muted'
            "
          >
          ...
          <span class="min-w-0">
            ...
            <span v-if="step.description" class="mt-0.5 block truncate text-xs text-muted">{{
              step.description
            }}</span>
          </span>
        </button>
        <span
          ...
          class="mx-1 hidden h-px w-6 shrink-0 bg-uzazure-200 lg:block"
          :class="step.status === 'complete' ? 'bg-uzazure-500' : 'bg-uzazure-100'"
          aria-hidden="true"
        />
      </li>
    </ol>
