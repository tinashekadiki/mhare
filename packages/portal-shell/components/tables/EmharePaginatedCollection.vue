...
<template>
  <div class="min-w-0" data-emhare-paginated-collection>
    ...
    <div class="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-muted pt-3 text-sm text-muted" data-emhare-pagination>
      <span>
        {{ items.length ? ((page - 1) * pageSize) + 1 : 0 }}–{{ Math.min(page * pageSize, items.length) }}
        of {{ items.length }} records · Page {{ page }} of {{ pageCount }}
      </span>
      <div class="flex flex-wrap items-center justify-end gap-2">
        <USelect v-model="pageSize" :items="pageSizeItems" class="w-36" aria-label="Records per page" />
        <UPagination
          :page="page"
