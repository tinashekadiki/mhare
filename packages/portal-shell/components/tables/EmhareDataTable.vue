...
const visibleColumnKeys = computed(() =>
  props.state.visibleColumns?.length
    ? props.state.visibleColumns
    : props.columns.filter((column) => !column.hidden).map((column) => column.key),
);

const visibleColumns = computed(() =>
  props.columns.filter((column) => visibleColumnKeys.value.includes(column.key)),
);
...
const selectedRows = computed(() =>
  props.rows.filter((row) => selectedKeys.value.includes(rowId(row))),
);
...
const savedViewItems = computed(() => [
  ...
  })),
  {
    ...
    icon: "i-lucide-save",
    onSelect: () => emit("saved-view-create", props.state),
  },
]);
...
<template>
  ...
  >
      ...
      <UDropdownMenu :items="bulkItems">
        <EmhareGuidedActionButton
          ...
          :guidance-instructions="
            selectedKeys.length
              ? []
              : ['Select at least one table row before opening bulk actions.']
          "
        />
      </UDropdownMenu>
      ...
      <table class="w-full min-w-max text-sm">
        <thead class="sticky top-0 z-10 bg-elevated text-left text-muted">
          <tr>
            ...
            <th v-if="rowActions.length" class="w-12 p-3 text-right">Actions</th>
          </tr>
        </thead>
            ...
            <tr v-if="expandable && isExpanded(row)" class="border-t border-muted bg-elevated/40">
              <td :colspan="visibleColumns.length + 3" class="p-4">
                <slot name="expanded-row" :row="row">
                  <pre class="overflow-auto rounded-md bg-default p-3 text-xs text-muted">{{
                    row
                  }}</pre>
                </slot>
              </td>
        ...
        <tfoot
          v-if="visibleColumns.some((column) => column.total)"
          class="border-t border-muted bg-elevated font-medium"
        >
          <tr>
    ...
    <div
      class="flex flex-wrap items-center justify-between gap-3 border-t border-muted p-3 text-sm text-muted"
      data-emhare-pagination
    >
      <span
        >{{ selectedKeys.length }} selected · {{ total }} total · Page {{ state.page }} of
        {{ pageCount }}</span
      >
