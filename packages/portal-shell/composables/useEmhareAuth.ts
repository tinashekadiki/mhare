...
} from "@emhare/portal-shell/utils/portal-routing";
...
async function userManager() {
  ...
  const portalKind = inferPortalKind(window.location);
  const settings: UserManagerSettings = {
    ...
    redirect_uri: `${origin}${portalCallbackPath(portalKind, window.location)}`,
...
export function useEmhareAuth() {
  ...
  const currentUserProfile = useState<CoreCurrentUserProfile | null>(
    "emhare-core-user-profile",
    () => null,
  );
  ...
  const displayName = computed(
    () =>
      currentUserProfile.value?.user.displayName ??
      oidcUser.value?.profile.name ??
      oidcUser.value?.profile.email ??
      "Operator",
  );
  const realmRoles = computed(() => new Set(currentUserProfile.value?.realmRoles ?? []));
  const localRoleCodes = computed(
    () =>
      new Set(
  ...
  const effectivePermissionCodes = computed(
    () => new Set(currentUserProfile.value?.effectivePermissionCodes ?? []),
  );
  ...
  const operationalAccess = computed(() => {
    ...
    return (
      [...realmRoles.value].some((role) => OPERATIONAL_REALM_ROLES.has(role)) ||
      Boolean(
        currentUserProfile.value?.roleAssignments.some(
  ...
  function hasPermissionPrefix(permissionPrefix: string) {
    ...
    return [...effectivePermissionCodes.value].some((permissionCode) =>
      permissionCode.startsWith(permissionPrefix),
    );
  }

  function assignedSelfServicePortalUrl() {
    const localRoleCodes = new Set(
      currentUserProfile.value?.roleAssignments.map((assignment) => assignment.roleCode) ?? [],
    );
    if (realmRoles.value.has("student") || localRoleCodes.has("STUDENT")) {
      return publicEnv("NUXT_PUBLIC_STUDENT_PORTAL_URL", portalDestinationUrl("student"));
    }
    return publicEnv("NUXT_PUBLIC_APPLICANT_PORTAL_URL", portalDestinationUrl("applicant"));
  }
  ...
  async function login(returnTo?: string) {
    ...
    if (!signInRedirectPromise) {
      signInRedirectPromise = (async () => {
        ...
        const candidate =
          requestedTargetPath === "/" && existingTargetPath
            ? existingTargetPath
            : requestedTargetPath;
        const targetPath = sanitizePortalReturnPath(
          candidate,
  ...
  async function signup(returnTo?: string) {
    ...
    const existingTargetPath = localStorage.getItem("emhare:returnTo");
    const candidate =
      requestedTargetPath === "/" && existingTargetPath ? existingTargetPath : requestedTargetPath;
    const targetPath = sanitizePortalReturnPath(
      candidate,
      window.location.origin,
      portalKind,
  ...
  async function handleCallback() {
    ...
    try {
      ...
      const returnTo = sanitizePortalReturnPath(
        callbackState ||
          sessionStorage.getItem("emhare:returnTo") ||
          localStorage.getItem("emhare:returnTo"),
        window.location.origin,
  ...
  async function syncCoreUser() {
    ...
    try {
      currentUserProfile.value = await $fetch<CoreCurrentUserProfile>(
        `${publicEnv("NUXT_PUBLIC_API_BASE", "http://localhost:8080")}/api/core/me`,
        {
          headers: {
