...
async function createFixture(): Promise<AcademicUiFixture> {
  ...
  const createUserResponse = await keycloakAdmin.post(
  ...
  const roleResponse = await keycloakAdmin.get(
...
async function createCrudFixture(): Promise<AcademicCrudFixture> {
  ...
  if (!academicUnitTypeRootId || !academicUnitTypeLeafId) {
    throw new Error(
      "Canonical FACULTY and DEPARTMENT academic unit types are required for the CRUD fixture.",
    );
  }
...
async function exposeFixtureProgrammeThroughApplicationTypes(
  page: Page,
  fixture: AcademicUiFixture,
) {
  await page.route("**/api/admissions/application-types/*/route-configuration", async (route) => {
    if (route.request().method() !== "GET") return route.continue();
    const response = await route.fetch();
    const configuration = await response.json();
    if (configuration.code !== "UNDERGRAD") return route.fulfill({ response });
    await route.fulfill({
      response,
      json: {
        ...configuration,
        programmes: [
          ...configuration.programmes.filter(
            (programme: { programmeId: string }) => programme.programmeId !== fixture.programmeId,
          ),
          {
            programmeId: fixture.programmeId,
            programmeCode: `B${fixture.codeSuffix.slice(0, 4)}`,
...
test.describe("Academic Setup operational UI", () => {
  test("guides intake creation through details, eligibility, and opening review", async ({
    page,
  }) => {
    let fixture: AcademicUiFixture | null = null;
    try {
      fixture = await createFixture();
      await exposeFixtureProgrammeThroughApplicationTypes(page, fixture);
      ...
      await intakeWorkspace
        .getByLabel("Applicant-facing name")
        .fill(`February ${fixture.calendarYear} Intake`);
      ...
      await intakeWorkspace
        ...
        .fill(`${fixture.calendarYear}-05-15`);
      ...
      await expect(
        intakeWorkspace.getByText("Managed in Application Types", { exact: true }),
      ).toBeVisible();
      await expect(
        intakeWorkspace.getByText(new RegExp(`B${fixture.codeSuffix.slice(0, 4)}`)),
      ).toBeVisible();
      ...
      await intakeWorkspace
        .getByLabel(`B${fixture.codeSuffix.slice(0, 4)} total capacity`)
        .fill("80");
      ...
      await expect(
        intakeWorkspace.getByRole("heading", { name: "Review and open applications" }),
      ).toBeVisible();
      await expect(
        intakeWorkspace.getByText(`February ${fixture.calendarYear} Intake`, { exact: true }),
      ).toBeVisible();
      await intakeWorkspace
        .getByLabel("Opening reason")
        .fill("Configured all admission opening controls for browser verification.");
      await expect(intakeWorkspace.getByRole("button", { name: "Save draft" })).toBeVisible();
      await expect(
        intakeWorkspace.getByRole("button", { name: "Create and open intake" }),
      ).toBeVisible();
      ...
      const intakeResponse = (status: "DRAFT" | "OPEN", version: number) => ({
        ...
        programmeLevels: [
          {
            id: fixture!.programmeLevelId,
            code: `UG_${fixture!.codeSuffix}`,
            name: "Undergraduate",
          },
        ],
        specificProgrammes: [],
      ...
      await page.route(
        "**/api/admissions/application-types/*/route-configuration",
        async (route) => {
      ...
      await page.route(
        `**/api/admissions/intakes/${createdIntakeId}/programme-quotas`,
        async (route) => {
      ...
      await expect(
        page.getByText(`February ${fixture.calendarYear} Intake configured and opened`, {
          exact: true,
        }),
      ).toBeVisible();
    ...
    try {
      ...
      await page.route("**/api/academic/overview", async (route) => {
        ...
        await route.fulfill({
          ...
          json: {
            ...overview,
            intakes: overview.intakes.filter(
              (intake: { id: string }) => intake.id === fixture!.intakeId,
            ),
          },
        });
      });
      ...
      const correctionReason =
        "Corrected the published intake dates after the approved calendar review.";
      ...
      const intakeRow = page
        .getByRole("row")
        .filter({ hasText: `January ${fixture.calendarYear} Intake` });
      await intakeRow.getByRole("button", { name: "Edit" }).click();
      const editIntakeDrawer = page.getByRole("region", { name: "Edit intake" });
  ...
  test("shows the academic period switcher only on scoped pages and filters admissions by the selected period", async ({
    page,
  }) => {
    let fixture: AcademicUiFixture | null = null;
    try {
      ...
      await page.route("**/api/academic/overview", async (route) => {
        ...
        await route.fulfill({
          ...
          json: {
            ...
            academicYears: [
              {
                id: firstAcademicYearId,
                name: "2026",
                startDate: "2026-01-01",
                endDate: "2026-06-30",
                status: "OPEN",
                changeReason: "Browser fixture",
                version: 0,
              },
              {
            ...
            academicPeriods: [
              ...
              {
                id: secondAcademicPeriodId,
                academicYearId: secondAcademicYearId,
      ...
      const workItem = (
        ...
        applicationId,
        applicationNumber,
        applicantNumber: `APP-${applicationNumber.slice(-4)}`,
        applicantName: `Applicant ${applicationNumber.slice(-4)}`,
        intakeId,
        intakeCode,
      ...
      await page.route("**/api/admissions/work-items?*", (route) => {
        const academicYearId = new URL(route.request().url()).searchParams.get("academicYearId");
        const content =
          academicYearId === firstAcademicYearId
            ? [workItem(firstApplicationId, "EMH-PERIOD-2026", firstIntakeId, "JAN-2026")]
            : academicYearId === secondAcademicYearId
              ? [workItem(secondApplicationId, "EMH-PERIOD-2027", secondIntakeId, "JAN-2027")]
              : [
                  workItem(firstApplicationId, "EMH-PERIOD-2026", firstIntakeId, "JAN-2026"),
                  workItem(secondApplicationId, "EMH-PERIOD-2027", secondIntakeId, "JAN-2027"),
                ];
        return route.fulfill({
          json: { content, page: 0, size: 25, totalElements: content.length, totalPages: 1 },
        });
      });
          ...
          json: [
            {
              ...
              moduleId: randomUUID(),
              moduleCode: "CSC-2026",
              moduleName: "Period One Module",
              academicPeriodId: firstAcademicPeriodId,
              academicPeriodCode: "2026-S1",
              academicPeriodName: "Semester 1",
              assignedInstructorUserId: fixture!.userId,
              status: "ACTIVE",
              version: 0,
              rosterCount: 12,
              schemes: [],
            },
            {
              ...
              moduleId: randomUUID(),
              moduleCode: "CSC-2027",
              moduleName: "Period Two Module",
              academicPeriodId: secondAcademicPeriodId,
      ...
      await page.route("**/api/assessment-results/roster-sources", (route) =>
        route.fulfill({ json: [] }),
      );
  ...
  test("renders governed structure, calendar, catalogues, and approved curriculum", async ({
    page,
  }, testInfo) => {
    let fixture: AcademicUiFixture | null = null;
    try {
      ...
      await expect(
        page.getByText("Department of Computing", { exact: true }).first(),
      ).toBeVisible();
      ...
      await page.screenshot({
        path: testInfo.outputPath("academic-structure.png"),
        fullPage: true,
      });
      ...
      await page.getByRole("tab", { name: /Intakes/ }).click();
      await expect(
        page.getByText(`January ${fixture.calendarYear} Intake`, { exact: true }),
      ).toBeVisible();
      ...
      const academicYearRowBeforeValidation = page
        .getByRole("row")
        .filter({ hasText: fixture.academicYearName });
      ...
      await expect(intakeWorkspace.getByLabel("Programme Levels")).toHaveCount(0);
      await expect(
        intakeWorkspace.getByText("Letters, numbers, hyphens, and underscores only.", {
          exact: true,
        }),
      ).toBeVisible();
      await intakeWorkspace.getByLabel("Academic year").click();
      ...
      await intakeWorkspace
        .getByLabel("Offer acceptance deadline")
        .fill(`${fixture.calendarYear}-03-15`);
      ...
      await page
        .getByRole("option", { name: new RegExp(`B${fixture.codeSuffix.slice(0, 4)}`) })
        .click();
      await page.keyboard.press("Escape");
      await expect(
        intakeWorkspace.getByText("Specific Programme whitelist", { exact: true }),
      ).toBeVisible();
      await intakeWorkspace.getByRole("button", { name: "Continue to routes and fees" }).click();
      await expect(intakeWorkspace.getByText("Step 3 of 5", { exact: true })).toBeVisible();
      await expect(
      ...
      await intakeWorkspace
        .getByLabel("Opening reason")
        .fill("Configured the complete intake opening for validation testing.");
      const [invalidIntakeResponse] = await Promise.all([
        page.waitForResponse(
          (response) =>
            response.url().endsWith("/api/academic/intakes") &&
            response.request().method() === "POST",
        ),
        intakeWorkspace.getByRole("button", { name: "Save draft" }).click(),
      ]);
      expect(invalidIntakeResponse.status()).toBe(400);
      expect(invalidIntakeResponse.request().postDataJSON().programmeLevelIds).toEqual([
        fixture.programmeLevelId,
      ]);
      expect(invalidIntakeResponse.request().postDataJSON().programmeIds).toEqual([
        fixture.programmeId,
      ]);
      ...
      await page.getByTestId("intake-wizard-actions").getByRole("link", { name: "Cancel" }).click();
      consoleErrors.length = 0;
      ...
      const intakeRowBeforeCorrection = page
        .getByRole("row")
        .filter({ hasText: initialIntakeName });
      ...
      await editIntakeDrawer
        .getByLabel("Change reason")
        .fill("Corrected the published intake display name.");
      await editIntakeDrawer.getByRole("button", { name: "Save changes" }).click();
      ...
      ).toBe("Corrected the published intake display name.");
      expect(
        executeSql(
          "emhare_academic_setup",
      ...
      await page
        .getByPlaceholder("Search code, Module, or owner")
        .fill(`CSC101_${fixture.codeSuffix}`);
      await expect(
        page.getByText("Programming Fundamentals", { exact: true }).first(),
      ).toBeVisible();

      await page.goto("/operations/programmes");
      await expect(page.getByRole("heading", { name: "Programme catalogue" })).toBeVisible();
      await page
        .getByPlaceholder("Search the current register")
        .fill(`B${fixture.codeSuffix.slice(0, 4)}`);
      await expect(
        page.getByText("Bachelor of Science in Information Technology", { exact: true }).first(),
      ).toBeVisible();
      ...
      await programmeSearch.fill(`B${fixture.codeSuffix.slice(0, 4)}`);
      await expect(
        page.getByRole("option", { name: new RegExp(`B${fixture.codeSuffix.slice(0, 4)}`) }),
      ).toBeVisible();
      await programmeSearch.press("Escape");
      await expect(
        page.getByText("Approved curriculum remains amendable", { exact: true }),
      ).toBeVisible();
      ...
      await page.screenshot({
        path: testInfo.outputPath("approved-curriculum.png"),
        fullPage: true,
      });
  ...
  test("keeps setup actions available and explains missing programme and Module prerequisites", async ({
    page,
  }) => {
    ...
    try {
      ...
      await page.route("**/api/academic/programmes/*/versions", (route) =>
        route.fulfill({
          json: [
            {
      ...
      await page.route("**/api/academic/programme-versions/*/curriculum", (route) =>
        route.fulfill({ json: [] }),
      );
      ...
      await expect(setupGuidance.locator("..")).toContainText(
        "Create an active academic unit using a hierarchy level that can own academic records",
      );
      ...
      await expect(page.getByRole("heading", { name: "Curriculum versions" })).toBeVisible();
      await expect(
        page.getByText("No Module is currently available to add", { exact: true }),
      ).toBeVisible();
      await expect(
        page
          .getByText("Create and activate at least one Module in the Module catalogue.", {
            exact: true,
          })
          .first(),
      ).toBeVisible();
  ...
  test("creates a Module, a programme, and its curriculum mapping end-to-end through the operator UI", async ({
    page,
  }) => {
    ...
    try {
      ...
      const moduleDrawer = page.getByRole("region", { name: "Create Module" });
      ...
      await moduleDrawer
        ...
        .fill("Created by the Playwright Programmes/Modules/Curriculum CRUD test.");
      await moduleDrawer.getByLabel("Credit value").fill("15");
      const [moduleCreateResponse] = await Promise.all([
        page.waitForResponse(
          (response) =>
            response.url().endsWith("/api/academic/modules") &&
            response.request().method() === "POST",
        ),
      ...
      const [moduleActivateResponse] = await Promise.all([
        page.waitForResponse((response) =>
          response.url().endsWith(`/api/academic/modules/${created.moduleId}/activate`),
        ),
      ...
      const [levelResponse] = await Promise.all([
        page.waitForResponse(
          (response) =>
            response.url().endsWith("/api/academic/programme-levels") &&
            response.request().method() === "POST",
        ),
      ...
      const [typeResponse] = await Promise.all([
        page.waitForResponse(
          (response) =>
            response.url().endsWith("/api/academic/programme-types") &&
            response.request().method() === "POST",
        ),
      ...
      const programmeDrawer = page.getByRole("region", { name: "Create programme" });
      await expect(
        programmeDrawer.getByRole("heading", { name: "Create programme" }),
      ).toBeVisible();
      ...
      const [programmeResponse] = await Promise.all([
        page.waitForResponse(
          (response) =>
            response.url().endsWith("/api/academic/programmes") &&
            response.request().method() === "POST",
        ),
      ...
      const programmeChangeReason =
        "Corrected the programme name and extended the maximum duration.";
      await programmeRow.getByRole("button", { name: "Edit" }).click();
      const editProgrammeDrawer = page.getByRole("region", { name: "Edit programme" });
      await expect(
        editProgrammeDrawer.getByRole("heading", { name: "Edit programme" }),
      ).toBeVisible();
      ...
      await page.waitForURL(
        new RegExp(`/operations/curriculum\\?programmeId=${created.programmeId}`),
      );
      await expect(page.getByRole("heading", { name: "Curriculum versions" })).toBeVisible();
      ...
      await expect(
        page.getByText("Draft curriculum version created", { exact: true }),
      ).toBeVisible();
      await expect(page.getByText(`Version ${versionCode}`)).toBeVisible();

      await page.getByRole("button", { name: "Add Module" }).click();
      const curriculumDrawer = page.getByRole("region", { name: "Add Module to curriculum" });
      ...
      await curriculumDrawer
        .getByLabel("Amendment reason")
        .fill("Added as part of the approved programme curriculum design.");
        ...
        page.waitForResponse(
          (response) =>
            response
              .url()
              .endsWith(
                `/api/academic/programme-versions/${created.programmeVersionId}/curriculum`,
              ) && response.request().method() === "POST",
        ),
      ...
      const [approveResponse] = await Promise.all([
        page.waitForResponse((response) =>
          response
            .url()
            .endsWith(`/api/academic/programme-versions/${created.programmeVersionId}/approve`),
        ),
        page.locator("button.swal2-confirm").click(),
      ]);
      ...
      const amendmentDrawer = page.getByRole("region", { name: `Amend ${moduleCode}` });
      await expect(
        amendmentDrawer.getByRole("heading", { name: `Amend ${moduleCode}` }),
      ).toBeVisible();
      await amendmentDrawer.getByLabel("Minimum pass mark").fill("55");
      await amendmentDrawer
        .getByLabel("Amendment reason")
        .fill("Approved by the curriculum committee after academic review.");
        ...
        page.waitForResponse(
          (response) =>
            response
              .url()
              .endsWith(
                `/api/academic/programme-versions/${created.programmeVersionId}/curriculum/${created.curriculumModuleId}`,
              ) && response.request().method() === "PUT",
        ),
      ...
      const [programmeActivateResponse] = await Promise.all([
        page.waitForResponse((response) =>
          response.url().endsWith(`/api/academic/programmes/${created.programmeId}/activate`),
        ),
        page.locator("button.swal2-confirm").click(),
      ]);
      ...
      const [retireResponse] = await Promise.all([
        page.waitForResponse((response) =>
          response
            .url()
            .endsWith(`/api/academic/programme-versions/${created.programmeVersionId}/retire`),
        ),
        page.locator("button.swal2-confirm").click(),
      ]);
