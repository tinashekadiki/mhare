...
test.afterAll(async () => {
  ...
  const result = spawnSync(
    "docker",
    ["exec", "-i", "emhare-postgres", "psql", "-q", "-U", "postgres", "-d", "emhare_core_identity"],
    {
...
async function login(
  page: Page,
  discountRoute: ((route: Route) => Promise<void> | void) | null = (route) =>
    route.fulfill({ json: { discounts: [] } }),
  applicantCategoryRoute: ((route: Route) => Promise<void> | void) | null = (route) =>
    route.fulfill({
      json: [
  ...
  if (applicantCategoryRoute)
    await page.route("**/api/admissions/applications/applicant-categories", applicantCategoryRoute);
...
function academicOverviewFixture() {
  ...
  return {
    ...
    overview: {
      ...
      academicYears: [
        {
          ...
          endDate: "2027-12-31",
          status: "OPEN",
          changeReason: "Opened for testing",
          version: 0,
        },
      ],
      academicPeriodTypes: [],
      ...
      programmeTypes: [
        { id: programmeTypeId, code: "DEGREE", name: "Degree", status: "ACTIVE", version: 0 },
      ],
      programmes: [
        {
          id: programmeId,
          ...
          owningAcademicUnitId: departmentId,
          owningAcademicUnitName: "Department of Accounting",
          programmeTypeId,
...
for (const viewport of [
  ...
  test(`presents governed fee definitions and effective pricing on ${viewport.name}`, async ({
    page,
  }) => {
      ...
      catalogues: [
        {
          ...
          description: "Approved tuition charge for registered undergraduate students.",
          ...
          status: "ACTIVE",
          preparedByUserId: randomUUID(),
          activatedByUserId: randomUUID(),
          activatedAt: "2027-01-03T08:00:00Z",
          version: 1,
          rules: [
            {
              id: draftRuleId,
              ...
              effectiveUntil: "2027-06-30T23:59:59Z",
              scopeSignature: null,
              status: "DRAFT",
              preparedByUserId: randomUUID(),
              version: 0,
              scopes: [
                {
                  ...
                  referenceCode: "BACC",
                  referenceName: "Bachelor of Accountancy",
                },
              ],
            },
            {
              ...
              scopes: [
                {
                  id: randomUUID(),
                  scopeDimension: "ACADEMIC_PERIOD",
                  referenceId: randomUUID(),
                  referenceCode: "2026-S2",
                  referenceName: "2026 Semester 2",
                },
              ],
            },
          ],
    ...
    await page.route("**/api/academic/overview", (route) =>
      route.fulfill({ json: academic.overview }),
    );
          ...
          structures: [
            {
              id: randomUUID(),
              code: "2027-S1-BACC",
              name: "BAcc semester 1 fees",
              description: "Complete programme fee schedule.",
              feeContext: "ACADEMIC",
              scopeType: "PROGRAMME",
              scopeReferenceId: academic.programmeId,
              scopeReferenceCode: "BACC",
              scopeReferenceName: "Bachelor of Accountancy",
              programmeLevelId: academic.programmeLevelId,
              programmeLevelCode: "UG",
              programmeLevelName: "Undergraduate",
              academicPeriodId: academic.academicPeriodId,
              academicPeriodCode: "2027-S1",
              academicPeriodName: "Semester 1",
              programmePeriodNumber: 1,
              applicantCategoryCode: null,
              transactionCurrencyCode: "USD",
              effectiveFrom: "2027-01-06T00:00:00Z",
              effectiveUntil: "2027-06-21T00:00:00Z",
              status: "ACTIVE",
              preparedByUserId: randomUUID(),
              activatedByUserId: randomUUID(),
              activatedAt: "2027-01-03T08:00:00Z",
              ...
              lines: [
                ...
                },
                {
                  ...
                  feeCode: "STUDENT-LEVY",
                  feeName: "Student levy",
                  description: "Student services levy",
                  chargeType: "PROGRAMME",
                  receivableAccountCode: "AR-STUDENT",
                  revenueAccountCode: "REV-LEVY",
                  taxCode: null,
    ...
    await page.getByRole("tab", { name: /Student fee structures/ }).click();
    ...
    await expect(
      structureRegister.getByText("Programme · BACC · Bachelor of Accountancy", { exact: true }),
    ).toBeVisible();
    ...
    const priceDrawer = page.getByRole("region", { name: "Add effective price · TUITION-UG" });
    await expect(priceDrawer.getByText("Add effective price · TUITION-UG")).toBeVisible();
    await expect(
      page.getByText(
        "A price is selected only when every scope dimension and effective date matches the billing event.",
      ),
    ).toBeVisible();
    await priceDrawer.getByRole("button", { name: "Back" }).click();
...
test("creates a complete academic fee structure with governed line items", async ({ page }) => {
  ...
  await page.route("**/api/finance/fee-catalogues", (route) =>
    route.fulfill({ json: { catalogues: [] } }),
  );
  await page.route("**/api/finance/fee-structures", async (route) => {
    if (route.request().method() === "POST") {
      submittedStructure = route.request().postDataJSON();
      await route.fulfill({
        json: { id: randomUUID(), ...submittedStructure, status: "DRAFT", version: 0, lines: [] },
      });
      return;
    }
  ...
  await login(page);
  await page.getByRole("tab", { name: /Student fee structures/ }).click();
  await page.getByRole("button", { name: "New student fee structure" }).click();
  const drawer = page.getByRole("region", { name: "Create fee structure" });
  ...
  expect(submittedStructure!.lines[0]).toMatchObject({
    feeCode: "TUITION",
    feeName: "Tuition",
    amount: 800,
  });
  expect(submittedStructure!.lines[1]).toMatchObject({
    feeCode: "STUDENT-LEVY",
    feeName: "Student levy",
    amount: 50,
  });
...
test("scopes an application fee by programme level and a portal applicant category", async ({
  page,
}) => {
  const academic = academicOverviewFixture();
  let submittedStructure: Record<string, any> | undefined;
  ...
  await page.goto("/operations/finance-fees");
  await login(page);
  await expect(page.getByRole("tab", { name: /Application fees/ })).toHaveAttribute(
    "aria-selected",
    "true",
  );
  await page.getByRole("button", { name: "Configure application fee" }).click();
  const formPage = page.getByRole("region", { name: "Configure application fee" });
  await expect(formPage).toHaveAttribute("data-emhare-form-presentation", "page");
  await expect(page.locator("#emhare-route-content")).toHaveAttribute("aria-hidden", "true");
  await expect(page.getByRole("button", { name: "Search" })).toBeVisible();
  if ((page.viewportSize()?.width ?? 0) >= 1024) {
    await expect(page.getByRole("link", { name: "Billing and invoices" })).toBeVisible();
  } else {
    await expect(page.getByRole("button", { name: "Open sidebar" })).toBeVisible();
  }
  await expect(formPage.getByText("Configure application fee", { exact: true })).toBeVisible();
  await expect(formPage.getByLabel("Fee class")).toHaveCount(0);
  await expect(formPage.getByLabel("Programme level")).toBeVisible();
  await expect(formPage.getByLabel("Applicant category")).toBeVisible();
  await formPage.getByLabel("Programme level").click();
  await page.getByRole("option", { name: "UG · Undergraduate" }).click();
  await formPage.getByLabel("Applicant category").click();
  await page.getByRole("option", { name: "Local applicant" }).click();
  await formPage.getByLabel("Structure code").fill("APP-UG-LOCAL");
  await formPage.getByLabel("Structure name").fill("Local undergraduate application fee");
  await formPage.getByLabel("Application fee amount").fill("25");
  await formPage.getByLabel("Application fee amount").press("Tab");
  await formPage.getByRole("button", { name: "Save draft application fee" }).click();
  ...
  expect(submittedStructure).toMatchObject({
    ...
    scopeReferenceId: academic.programmeLevelId,
    scopeReferenceCode: "UG",
    scopeReferenceName: "Undergraduate",
    programmeLevelId: academic.programmeLevelId,
    programmeLevelCode: "UG",
    programmeLevelName: "Undergraduate",
...
test("configures a standalone institution attachment discount", async ({ page }) => {
    ...
    if (route.request().method() === "POST") {
      ...
      await route.fulfill({
        json: {
          ...
          status: "DRAFT",
          version: 0,
          applicableProgrammes: submittedDiscount?.applicableProgrammes ?? [],
        },
      });
      return;
    }
  ...
  await expect(page.getByRole("tab", { name: /Student discounts/ })).toHaveAttribute(
    "aria-selected",
    "true",
  );
  await expect(page.getByText("One matching discount wins")).toBeVisible();
  await page.getByRole("button", { name: "New student discount" }).click();
  const drawer = page.getByRole("region", { name: "Create student discount" });
  ...
  await drawer.getByLabel("Programme level").click();
  await page.getByRole("option", { name: "UG · Undergraduate" }).click();
  await drawer.getByLabel("Programme study level").click();
  await page.getByRole("option", { name: "3.1 · Year 3 · Semester 1" }).click();
  await drawer.getByLabel("Discount rate").fill("15");
...
test("loads the live fee structure workspace through the gateway", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Application fee register" })).toBeVisible();
  await expect(page.getByText("Application payment gate")).toBeVisible();
  await page.getByRole("button", { name: "Configure application fee" }).click();
  const applicationFeeFormPage = page.getByRole("region", { name: "Configure application fee" });
  await expect(applicationFeeFormPage).toHaveAttribute("data-emhare-form-presentation", "page");
  await expect(page.getByRole("button", { name: "Search" })).toBeVisible();
  await expect(
    applicationFeeFormPage.getByText("Configure application fee", { exact: true }),
  ).toBeVisible();
  await expect(applicationFeeFormPage.getByLabel("Programme level")).toBeVisible();
  await expect(applicationFeeFormPage.getByLabel("Applicant category")).toContainText(
    "All applicant categories",
  );
  await applicationFeeFormPage.getByLabel("Applicant category").click();
  ...
  await expect(
    page.getByRole("option", { name: "Continuing legal education applicant" }),
  ).toBeVisible();
  await page.keyboard.press("Escape");
  await applicationFeeFormPage.getByRole("button", { name: "Back" }).click();
  await expect(applicationFeeFormPage).toBeHidden();
  await expect(page.locator("#emhare-route-content")).not.toHaveAttribute("aria-hidden", "true");

  await page.getByRole("tab", { name: /Student fee structures/ }).click();
  await expect(page.getByRole("heading", { name: "Fee structure register" })).toBeVisible();
  await expect(page.getByText("One complete schedule wins")).toBeVisible();
  await page.getByRole("button", { name: "New student fee structure" }).click();
  const formPage = page.locator('[data-emhare-form-presentation="page"]');
  await expect(formPage).toBeVisible();
  await expect(formPage.getByText("Structure purpose")).toBeVisible();
  await expect(formPage.getByText("Fee line items")).toBeVisible();
  await expect(formPage.getByLabel("Scope level")).toBeVisible();
  await formPage.getByRole("button", { name: "Back" }).click();
  await expect(formPage).toBeHidden();
  await page.getByRole("tab", { name: /Student discounts/ }).click();
  await page.getByRole("button", { name: "New student discount" }).click();
  const discountFormPage = page.getByRole("region", { name: "Create student discount" });
  await expect(discountFormPage).toHaveAttribute("data-emhare-form-presentation", "page");
  await expect(
    discountFormPage.getByRole("heading", { name: "Student applicability" }),
  ).toBeVisible();
  await expect(discountFormPage.getByLabel("Programme level")).toBeVisible();
  await expect(discountFormPage.getByLabel("Programme study level")).toBeVisible();
  await expect(discountFormPage.getByText("The student's current year and semester")).toBeVisible();
  await discountFormPage.getByRole("button", { name: "Back" }).click();
  await expect(discountFormPage).toBeHidden();
  expect(errors).toEqual([]);
});

test("presents the governed billing, collections, corrections, and student account workspaces", async ({
  page,
}) => {
  ...
  const billingRegister = {
    billingPolicies: [
      {
        ...
        code: "REG-TUITION",
        policyVersion: 1,
        name: "Registration tuition",
        sourceEventType: "REGISTRATION_CONFIRMED",
        feeCatalogueId: randomUUID(),
        feeCode: "TUITION-UG",
        feeName: "Undergraduate tuition",
    ...
    billingEvents: [
      {
        id: randomUUID(),
        eventNumber: "EMH-BEV-0000000042",
        sourceService: "student-records-service",
        sourceEventType: "REGISTRATION_CONFIRMED",
        sourceEventId: randomUUID(),
        sourceAggregateType: "REGISTRATION",
        sourceAggregateId: randomUUID(),
        sourceLineReference: "2026-S2-REGISTRATION",
        studentFinanceAccountId: accountId,
        accountNumber: "EMH-SFA-0000000042",
        studentId: randomUUID(),
        studentNumber: "R260042A",
        feeCatalogueId: randomUUID(),
        feeCode: "TUITION-UG",
        feeName: "Undergraduate tuition",
        feeRuleId: randomUUID(),
        feeRuleVersion: 1,
        description: "2026 semester 2 registration tuition",
        quantity: 1,
        transactionCurrencyCode: "USD",
        transactionUnitAmount: 100,
        ...
        preparedByUserId: randomUUID(),
        submittedAt: "2026-08-02T08:01:00Z",
    ...
    invoices: [
      {
        id: invoiceId,
        invoiceNumber: "EMH-INV-0000000042",
        studentFinanceAccountId: accountId,
        accountNumber: "EMH-SFA-0000000042",
        studentId: randomUUID(),
        studentNumber: "R260042A",
        ...
        lines: [
          {
            id: invoiceLineId,
            lineNumber: 1,
            billingEventId: randomUUID(),
            billingEventNumber: "EMH-BEV-0000000041",
            feeCode: "TUITION-UG",
  ...
  const collectionsRegister = {
    ...
    payments: [
      {
        id: paymentId,
        paymentNumber: "EMH-SPY-0000000042",
        studentFinanceAccountId: accountId,
        accountNumber: "EMH-SFA-0000000042",
        payerName: "Tariro Moyo",
        providerCode: "BANK",
        providerTransactionReference: "BANK-000042",
        ...
        reconciledByUserId: randomUUID(),
        reconciledAt: "2026-08-04T11:00:00Z",
        inSuspense: false,
        reversed: false,
        receiptNumber: "EMH-SRC-0000000042",
        version: 1,
      },
    ],
    receipts: [
      {
        ...
        studentFinanceAccountId: accountId,
        accountNumber: "EMH-SFA-0000000042",
        issuedAt: "2026-08-04T11:00:00Z",
      },
    ],
    allocations: [
      {
        id: randomUUID(),
        allocationNumber: "EMH-SAL-0000000042",
        paymentId,
        paymentNumber: "EMH-SPY-0000000042",
        invoiceId,
        invoiceNumber: "EMH-INV-0000000042",
        transactionCurrencyCode: "USD",
        transactionAmount: 40,
        paymentBaseAmount: 40,
        invoiceBaseAmount: 40,
        realisedExchangeDifference: 0,
        allocatedByUserId: randomUUID(),
        allocatedAt: "2026-08-04T11:05:00Z",
        reversed: false,
        reversalNumber: null,
        version: 0,
      },
    ...
    creditNotes: [
      {
        id: randomUUID(),
        creditNoteNumber: "EMH-SCN-0000000042",
        invoiceId,
        invoiceNumber: "EMH-INV-0000000042",
        transactionCurrencyCode: "USD",
        ...
        status: "DRAFT",
        preparedByUserId: randomUUID(),
        preparedAt: "2026-08-05T08:00:00Z",
        postedByUserId: null,
        postedAt: null,
        version: 0,
        lines: [
  ...
  const account = {
    ...
    studentId: randomUUID(),
    studentNumber: "R260042A",
    primaryEmail: "tariro.moyo@example.test",
    status: "ACTIVE",
    baseBalance: 50,
  };
  const statement = {
    ...
    lines: [
      {
        lineType: "INVOICE",
        reference: "EMH-INV-0000000042",
        occurredAt: "2026-08-02T09:00:00Z",
        description: "Posted student invoice",
        transactionCurrencyCode: "USD",
        ...
        baseDebit: 100,
        baseCredit: 0,
        runningBaseBalance: 100,
      },
      {
        lineType: "PAYMENT",
        reference: "EMH-SPY-0000000042",
        occurredAt: "2026-08-04T11:00:00Z",
        description: "Reconciled student payment",
        transactionCurrencyCode: "USD",
        transactionDebit: 0,
        transactionCredit: 40,
        baseCurrencyCode: "USD",
        baseDebit: 0,
        baseCredit: 40,
        runningBaseBalance: 60,
      },
      {
        lineType: "CREDIT_NOTE",
        reference: "EMH-SCN-0000000041",
        occurredAt: "2026-08-05T09:00:00Z",
        description: "Posted fee correction",
        transactionCurrencyCode: "USD",
        transactionDebit: 0,
  ...
  await page.route("**/api/academic/overview", (route) =>
    route.fulfill({ json: academicOverviewFixture().overview }),
  );

  await page.goto("/operations/finance-fees");
  await login(page);
  await page.route("**/api/finance/billing", (route) => route.fulfill({ json: billingRegister }));
  await page.route("**/api/finance/collections", (route) =>
    route.fulfill({ json: collectionsRegister }),
  );
  await page.route("**/api/finance/collections/accounts", (route) =>
    route.fulfill({ json: [account] }),
  );
  await page.route(`**/api/finance/collections/accounts/${accountId}/statement`, (route) =>
    route.fulfill({ json: statement }),
  );
