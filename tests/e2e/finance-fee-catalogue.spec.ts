...
test('scopes an application fee by programme level and a portal applicant category', async ({ page }) => {
  ...
  await page.goto('/operations/finance-fees')
  await login(page)
  await expect(page.getByRole('tab', { name: /Application fees/ })).toHaveAttribute('aria-selected', 'true')
...
test('configures a standalone institution attachment discount', async ({ page }) => {
  ...
  expect(submittedDiscount).toMatchObject({
    code: 'ATTACHMENT-2027', academicUnitDepth: 0,
    targetType: 'ALL_FEES', feeCatalogueId: null, discountPercentage: 15,
    authorityReference: 'Finance Committee minute FC-2027-04',
    programmeLevelId: academic.programmeLevelId, programmeLevelCode: 'UG',
    programmeLevelName: 'Undergraduate', programmeStudyLevel: '3.1'
  })
})
...
test('loads the live fee structure workspace through the gateway', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Search' })).toBeVisible()
  await expect(applicationFeeFormPage.getByText('Configure application fee', { exact: true })).toBeVisible()
  await expect(applicationFeeFormPage.getByLabel('Programme level')).toBeVisible()
  await expect(applicationFeeFormPage.getByLabel('Applicant category')).toContainText('All applicant categories')
  ...
  await expect(discountFormPage.getByRole('heading', { name: 'Student applicability' })).toBeVisible()
