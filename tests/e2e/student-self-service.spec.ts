...
async function login(page: Page, fixture: StudentSelfServiceFixture) {
  ...
  await page.goto(studentPortalUrl)
  await page.locator('#username').fill(email)
  await page.locator('#password').fill(testPassword)
  await page.locator('#kc-login').click()
  await page.waitForURL(`${studentPortalUrl}/**`, { timeout: 30_000 })
  await page.waitForLoadState('networkidle')
}

test.describe('Student self-service workspace', () => {
  test('uses Year and Semester while preserving governed registration ownership', async ({ page }, testInfo) => {
    ...
    const studentNumberCard = page.getByText('Institutional identifier', { exact: true }).locator('..')
    await expect(studentNumberCard.getByText(`STU-SELF-${fixture.runId.slice(0, 8)}-1`, { exact: true })).toBeVisible()
    ...
    await page.screenshot({ path: testInfo.outputPath('student-self-service.png'), fullPage: true })
