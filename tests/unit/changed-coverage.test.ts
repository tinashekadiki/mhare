...
describe("changed coverage input collection", () => {
  it("collects added lines and ignores deleted-only hunks", () => {
    const changedLines =
      parseUnifiedDiff(`diff --git a/services/example/src/main/java/example/Rule.java b/services/example/src/main/java/example/Rule.java
...
`);
    ...
    expect([...changedLines.keys()]).toEqual(["services/example/src/main/java/example/Rule.java"]);
    expect([...changedLines.values()][0]).toEqual(new Set([9, 10]));
  });

  it("ignores formatting-only line wrapping while retaining semantic changes", () => {
    const changedLines =
      parseUnifiedDiff(`diff --git a/services/example/src/main/java/example/Rule.java b/services/example/src/main/java/example/Rule.java
--- a/services/example/src/main/java/example/Rule.java
+++ b/services/example/src/main/java/example/Rule.java
@@ -8 +8,2 @@
-return resolve(longArgument, anotherArgument);
+return resolve(
...
`);
    ...
    expect(changedLines.get("services/example/src/main/java/example/Rule.java")).toEqual(
      new Set([16]),
    );
  });

  it("maps semantic word additions to their new source lines", () => {
    const changedLines =
      parseWordDiff(`diff --git a/services/example/src/main/java/example/Rule.java b/services/example/src/main/java/example/Rule.java
--- a/services/example/src/main/java/example/Rule.java
+++ b/services/example/src/main/java/example/Rule.java
@@ -8,2 +8,3 @@
 return resolve(
...
`);
  ...
  it("identifies changed Java modules that did not produce JaCoCo XML", () => {
    ...
    expect(
      findMissingBackendCoverageModules(changedLines, new Set(["services/admissions-service"])),
...
<report name="example">
  <package name="example/domain">
    ...
    expect(
      coverage.get("services/example/src/main/java/example/domain/Rule.java")?.get(10),
    ).toEqual({
      lineCovered: true,
      branchesCovered: 2,
      branchesTotal: 2,
    ...
    expect(
      coverage.get("services/example/src/main/java/example/domain/Rule.java")?.get(11),
    ).toEqual({
      lineCovered: false,
      branchesCovered: 1,
      branchesTotal: 2,
...
`,
    ...
    const result = evaluateChangedCoverage(new Map([[filePath, new Set([10, 11])]]), coverage);
...
describe("changed coverage evaluation", () => {
  ...
  it("does not penalise changed executable code when no branches exist", () => {
    ...
    const result = evaluateChangedCoverage(
      ...
      new Map([
        [filePath, new Map([[3, { lineCovered: true, branchesCovered: 0, branchesTotal: 0 }]])],
      ]),
    );
